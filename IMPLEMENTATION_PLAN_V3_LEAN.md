...
## N4-0. 목표 신호 스펙 — **확정 (2026-08-12, 사용자 결정)**

> 이 절이 없어서 이번 회차 내내 "실 `.sig` 를 따라야 할 제약" 으로 다뤘다. 실제로는
> **우리가 정하는 것** 이고 `.sig` 를 수정해도 된다. 기록이 없던 것이 결함이었다.

| 항목 | 값 |
|---|---|
| 주기 | **150 s — 제어 15 SC 전부 통일** |
| 현시 | **dual-ring 4현시** |
| 현시 순서 | **major 직진 → major 좌 → minor 직진 → minor 좌** |
| `green_min` | **20 s** (현시당) |
| `.sig` | **수정 가능.** 위 스펙에 맞춰 재작성한다 |

**유도되는 값.**

```
clearance  현시 전이당 3 s (실 .sig 실측)  →  lost_time = 4 x 3 = 12 s
effective_green_total = 150 - 12 = 138 s
green_min x 4 = 80 <= 138                     여유 58 s
green_max = 138 - 3 x 20 = 78 s
```
...
| | 현재 | 목표 |
|---|---|---|
| 주기 | 140·150·160·170 혼재 (150 이 7/15) | **150 통일** |
| 모델 현시 | **2** (`MODEL_PHASES = ("p1","p2")`) | **4** |
| 모델 `cycle_length` | 120.0 (러너 합성) | **150** |
| 모델 `lost_time` | 10.0 = 2 x (amber 3 + all_red 2) | **12 = 4 x 3** |
| `green_min` | 20.0 | 20 (변경 없음) |
| 러너 clearance | amber 3 + all_red 2 = 5 s | **3 s** |

**실 `.sig` 는 이미 dual-ring 이다.** SC1001 `mor_peak` 실측이다.

```
링 A   EBT 45 -> WBL 24  ‖  SBT 40 -> NBL 29   = 138
링 B   WBT 45 -> EBL 24  ‖  NBT 51 -> SBL 18   = 138
배리어  major 69 / minor 69      clearance 4 x 3 = 12      150 = 138 + 12
```

### 개정 (2026-08-12, 사용자 결정) — 현시 수는 유도된다

"15 SC 전부 4현시" 는 틀렸다. **현시는 그 현시에 녹색을 받는 SG 가 있을 때만 존재한다.**

    SC107   SBT·NBT 영구적색      -> minor 직진 없음    -> N = 3
    SC108   NBL·SBL 영구적색      -> minor 좌 없음      -> N = 3
    SC109   WBL·NBL·SBT·NBT 영구적색                    -> N = 3
    나머지 12 SC                                        -> N = 4

**유도값이 N 에 따라 달라진다.**

    lost_time             = N x 3
    effective_green_total = 150 - N x 3        (N=4 -> 138,  N=3 -> 141)
    green_min             = 20 (존재하는 현시마다)
    green_max             = eff - (N-1) x 20   (N=4 -> 78,  N=3 -> 101)

**재배분은 배리어 총량을 보존한다.** 배리어 안에서는 자유롭게 나눈다. 이 규칙으로
green_min 을 못 지키는 SC 가 나오면 **그 SC 의 현시 수를 다시 세어라** - 대개 영구적색 SG 를
살아 있는 현시로 잘못 센 것이다.

영구적색 SG 는 136 개 중 **20 개** 다(SC5 12 · SC107 2 · SC108 2 · SC109 4).

**다만 minor 배리어에서 두 링이 갈린다**(lead-lag 좌회전). 위 스펙의 "4현시" 는 두 링을
동기화한 것이므로 **채택하면 lead-lag 이 사라진다.** 이것은 의도된 단순화다.
...
> 그 값이 그대로 잡음 바닥이 된다.
>
> **숫자가 이것을 확인한다.** 실제 부모 4셀의 J(TTT veh·h)를 재 보면 anchor 900 에서
> `[1835.08, 2777.56]`, 즉 셀간 격차가 **942 veh·h** 다. base replay 가 시드를 바꾸는
> 것이었다면 `eps_J_vissim` 이 이 규모로 나오고, 그러면 하한 `1e-6` 은 아무 의미가 없으며
> 어떤 `ΔJ` 도 재료로 인정받지 못한다. 하한 `1e-6` 은 **"같은 것을 다시 돌리면 움직이면
> 안 된다"** 는 뜻일 때만 성립한다.
>
> #### 720회가 아니라 180회다
>
...
## N6b. plant 가 VISSIM 을 예측하는가 — 2026-08-15 신설

N6 본문은 **파라미터 추정**만 요구한다(jam density, 표본 수, CI 반폭). "우리 plant 로
rollout 했을 때 VISSIM 을 얼마나 맞히는가" 는 계획 어디에도 없었고 아무도 재지 않았다.
기존 `scripts/audit_real_world_prediction_accuracy.py` 는 **one-step 전용**이다
(`mode = coupled_interval_one_step`). 사용자 지적으로 이 절을 신설한다.

### 왜 부모런이 이 검증에 맞는가

부모런은 **no-control** 이다. VISSIM 궤적이 컨트롤러 정책에 오염되지 않았고 모델도
무제어로 굴리므로 **동역학만** 비교된다. 어댑터 정본 경로를 그대로 타고
(`traffic_state_from_vissim` → `demand_from_state` → `_guarded_no_control_baseline` →
`evaluate_price_point(depth_override=H).states`) 우회 호출을 만들지 않는다(N7 규약).

### 해상도를 낮출수록 좋아 보인다 — 세 층을 다 봐야 한다

MPC horizon(3스텝 = 180 s) 기준, 부모런 9셀:

| 층 | 지표 | 중앙값 APE |
|---|---|---:|
| 망 전체 집계 | 전체 대수 | 13.2% (보정 후 **3.0%**) |
| 망 전체 집계 | 도시부 | 19.8% (보정 후 **3.9%**) |
| 고속도로 세그먼트 16개 | 대수·밀도 | **15.5%** |
| 고속도로 세그먼트 | 속도 | 6.1% |
| **도시 구간(저류+큐) 119통** | 점유 | **51.7%** |

**집계가 맞는 것은 링크별 오차가 상쇄되기 때문이다.** 집계 스케일로 3% 를 만들어도
구간 하나하나는 50% 틀린다. 논문에서 "plant 가 VISSIM 을 재현한다" 를 주장하려면
**세 층을 같이 보고해야** 하고, 집계 숫자만 쓰면 과장이다.

임계는 저장소가 이미 쓰는 G5 기준(`max(5 veh, 0.10 × 관측)`)을 그대로 쓴다. 현재
구간 층 **통과율 29%**.

### 구간 오차의 원인 — 투영이 in-transit 예약을 잃는다

`traffic_state_from_vissim` 은 저류 **점유량만** 싣고 `urban_storage_release_buffer` 를
비워 둔다. 그 버퍼에 쓰는 코드는 plant 내부 두 곳뿐이고(`urban_queue_model.py:578-579`,
`:1162-1163`) 어댑터는 하나도 안 채운다. 그런데 방출이 전적으로 그 버퍼에 물려 있다.

```
내부 링크   released = _pop_buffer(...) 가 >0 일 때만 available 복원  (:962-973)
            -> 버퍼가 비면 영원히 안 빠진다. 유입만 쌓여 +114~208%
sink(_out)  arrived = occupancy - _pending_in_transit(...)          (:994)
            -> 버퍼가 비면 전량이 즉시 방출 후보. 정확히 -100%
```

sink 코드 주석이 그 뺄셈을 넣은 이유를 적어 놨다 — 안 빼면 "진입 substep 에 곧바로
이탈해 out 링크 통행시간이 0". **버퍼가 비면 그 옛 버그가 그대로 재현된다.**

실측이 이를 못 박는다 — 스텝 0(초기상태) **0.0%**, 스텝 1(60 s) **54.8%**, 그 뒤 평평.
투영은 정확하고 한 스텝 만에 재배치된다. 총량은 보존된다(부호 중앙값 +0.0%).

### 기각한 가설 셋

1. **장부 내부 재분류 artifact** — 보존 단위(저류 + `origin` 이 그 저류인 movement 큐)로
   재도 줄지 않는다(54.9% 대 저류만 51.0%). 오히려 조금 높다.
2. **용량 하한 10.0 이 원인** — 같은 `--storage-override` 로 재유도하니 하한에 걸린 23통 중
   jam 168 로 10 이상 회복하는 것은 6개뿐, 링크가 없는 통은 0개. 나머지는 진짜로 짧다.
3. **용량이 29% 작아 투영이 차를 버린다** — `-29.4%` 와 jam 과소 29% 가 맞아떨어져 유력해
   보였다. 저류 용량 161통을 jam168 판으로 덮어써 시험 → `-29.1%`. **차이 없다.**

### warm-start — 우리 버그분을 걷어낸다

버퍼를 손으로 지어 넣으면 유출만 복원되어 **더 나빠진다**(`-37.4%`). 대신 **plant 가
스스로 짓게** 한다. 그리고 런타임에는 과거 관측이 없으므로 현재 관측에서 W 초를 굴려
스케줄만 짓고 점유량은 관측값으로 되돌리는 변형을 쓴다(스케줄은 같은 비율로 축소해
모양 보존).

| 조건 | 스텝1 | G5 통과 | 부호 |
|---|---:|---:|---:|
| 기준선 | 51.7% | 29.2% | −25.8% |
| 과거관측 warm 900 | 40.9% | 39.7% | −11.3% |
| **self warm 900 (런타임 가능)** | **42.9%** | **38.8%** | **−6.7%** |
| 어댑터 내장 `RW_WARMSTART_SEC=900` | 44.9% | 32.7% | **−6.7%** |

```
~11 포인트   우리 투영 결함.  warm-start 로 걷힌다
~43%        남는 것.  plant 대 VISSIM 의 실재하는 동역학 오차
```

**검증 도구 얘기가 아니다.** 어댑터는 **결정마다** VISSIM 에서 상태를 다시 짓는다. 즉
지금까지 모든 제어 결정이 빈 버퍼 상태에서 후보를 채점해 왔다. 비용은 모델 한 스텝
0.21 s × 15 = 결정당 약 3 s 로 60 s 주기 안이다. 기본은 꺼짐이고 실런 A/B 로 제어
품질이 실제로 달라지는지 확인한 뒤에 켤지 정한다.

### 낡은 예측 캘리브레이션은 고칠 게 아니라 꺼야 한다

`real_world_prediction_calibration_pshb4500fix_20260724.json`(2026-07-24, 옛 망)이
`crossgate_predcal_20260724.json` 을 통해 pedovrx 격자에 `enabled: True` 로 상속돼 있다.

| 지표 | 낡은 스케일 | rollout 실측 |
|---|---:|---:|
| 도시부 | ~0.18 | **1.24** |
| 고속 속도 | 0.613 | 0.898 |

7배 어긋나고 방향도 반대다. **낡은 게 아니라 다른 망의 값**이다. 지금은 소비 경로
(`component_penalty`, terminal fit)가 이 격자에서 꺼져 있어 보고 숫자만 오염시키지만
`enabled` 인 채라 둘 중 하나만 켜지면 즉시 점수에 들어간다.

### 도구

```
scripts/audit_rollout_prediction_accuracy.py   집계 4지표, horizon 안에서 캘리브레이션 적합
scripts/audit_rollout_spatial_accuracy.py      세그먼트/저류 분해 (주의: 관측을 소유권
                                               지도로 모아 투영 불일치가 섞인다)
scripts/diagnose_storage_redistribution.py     양쪽에 같은 투영. 투영 대 동역학을 가른다
scripts/audit_rollout_conserved_unit.py        보존 단위 + G5 임계
scripts/audit_rollout_warmstart.py             warm-start 변형 비교
```
