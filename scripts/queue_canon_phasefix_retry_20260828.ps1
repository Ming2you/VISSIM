<#
2026-08-28. phasefix 재발사. dpoff 는 이미 완주(4775.8, 무제어 대비 -43.7).

첫 시도 실패: install_merged_movements 가 ValueError 로 3회 다 세웠다 — 커넥터 근거는
물리 회전 하나를 가리키는데 모델은 그 회전을 출구별로 쪼개 놨다. 하나만 고치면 병합
그룹 안에서 phase 가 갈린다(충돌 5건). 가드가 옳게 작동한 것이고, 보정을 그룹 단위로
걸도록 고쳤다(형제 5 동반 이동 · group_split 0 · 병합 통과 확인).

이 파일은 queue_canon_20260828.ps1 에서 arms 절만 바꿔 **바이트 단위로** 만들었다.
텍스트로 왕복하면 CRLF 가 겹쳐 백틱 줄이음이 깨진다(2026-08-28 에 두 번 겪었다).
...
$arms = @(
  @{ name = "canon_phasefix_20260828"; tuning = "evaluation/configs/canon_phasefix_20260828.json"; note = "dpoff + 선언 phase 보정 11건(병합 형제 5 포함)" }
)
