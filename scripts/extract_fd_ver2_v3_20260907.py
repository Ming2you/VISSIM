...
def main():
    ...
    enter = {}      # (dir, seg, t) -> 사슬 밖에서 그 셀로 들어온 대수(on-ramp 합류)
    ...
    with io.open(fzp, encoding="utf-8", errors="replace") as f:
        ...
        for line in f:
            ...
            if t != tcur:
                _flush(cur, prev, tcur, prev_t, meta, cross, leave, enter, t_min)
    ...
    for (d, si, t), a in sorted(acc.items()):
        ...
        en = enter.get((d, si, t), 0)
        qedge = cr * 720.0 / lanes
        qsend = (cr + lv) * 720.0 / lanes
        lanemix = ",".join("%d:%d" % (ln, n) for ln, n in sorted(a[2].items()))
        rows.append((d, si, t, a[0], lanes, round(k, 4), round(v, 3), round(qkv, 2), cr, lv, en,
                     round(qedge, 1), round(qsend, 1), round(en * 720.0, 1), lanemix))
    with io.open(outc, "w", encoding="utf-8", newline="") as g:
        g.write("dir,seg,t,n,lanes,k_veh_km_lane,v_kph,q_kv_veh_h_lane,cross,leave,enter,"
                "q_edge_veh_h_lane,q_send_veh_h_lane,q_ramp_veh_h,lane_mix" + chr(10))
        for r in rows:
            g.write(",".join(str(x) for x in r) + "\n")
    print("표본 %d 행 → %s" % (len(rows), outc))


def _flush(cur, prev, tcur, prev_t, meta, cross, leave, enter, t_min):
    ...
    for v, c2b in cur.items():
        # 합류: 직전에 사슬 밖(램프 링크)이거나 망에 없었는데 지금 사슬 위에 있다.
        if c2b is None: continue
        c1b = prev.get(v, "new")
        if c1b is not None and c1b != "new" and c1b[0] == c2b[0]: continue
        d2, cp = c2b
        L2 = meta[d2]["seglen_m"]; n2 = meta[d2]["nseg"]
        s2 = min(n2 - 1, int(cp // L2))
        if c1b == "new" and s2 == 0: continue      # 본선 원점 진입은 합류가 아니다
        enter[(d2, s2, tcur)] = enter.get((d2, s2, tcur), 0) + 1
