...
NET = R / "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn_qc.inpx"
LOG = R / "evaluation/runs/chain_gated_20260905.log"
GATE = 50.0
SEED = 13
FULL = 37
...
def make_config(tags):
    name = "g_" + ("_".join(tags) if tags else "base") + "_20260905"
...
def run(name, cfg_name):
    ...
    if outdir.exists():
        ...
        log("  부분 런 삭제 %s" % name)
    ps = ("& '%s' -Name '%s' -OutDir '%s' -Network '%s' -Tuning '%s' -Controller 'wu-link' -SimPeriod 5400 "
          "-ControlIntervalSec 150 -Seed %d -ControlStartSec 900 -WarmupController 'no-control' "
          "-StateLogIntervalSec 30 -DemandScale 1.0") % (RUNNER, name, outdir, NET, CFG / ("%s.json" % cfg_name), SEED)
...
def main():
    log("=== 관문 체인 v7 시작 · GATE %.0f · seed %d · 기준 → RL → RL+RS+B(B4c v2) → +uni1800 → 판단 ===" % (GATE, SEED))
    platform = []
    ttt = {}
    ttt["base"] = run("g1_base_x18_20260905", make_config(platform))
    log("  TTT 기준 = %.1f" % ttt["base"])
    ref_ttt, ref_run = ttt["base"], "g1_base_x18_20260905"
    # ① RL (완주 런 재사용)
    ttt["RL"] = run("g2_RL_x18_20260905", make_config(["RL"]))
    log("  TTT RL = %.1f (기준 대비 %+.1f)" % (ttt["RL"], ttt["RL"] - ttt["base"]))
    platform.append("RL")
    if ttt["RL"] <= ttt["base"] + GATE:
        ref_ttt, ref_run = ttt["RL"], "g2_RL_x18_20260905"
        log("  관문: RL 채택 → 플랫폼 = 정본+RL")
    else:
        log("  관문: RL 단독은 기각(기준 대비 %+.1f) — B 의 전제이므로 플랫폼에 두고 관문은 기준선(%.1f) 대비" % (ttt["RL"] - ttt["base"], ttt["base"]))
    # ② RS + B(B4c v2): 램프 신호 가격 0 + 착지 접근로 점유 + leg_split/offramp_direct. 관문은 기준선 대비.
    platform.append("RS")
    ttt["B"] = run("g4_B_x18_20260905", make_config(platform + ["B"]))
    log("  TTT RS+B4c(%s 위) = %.1f (기준 대비 %+.1f)" % ("+".join(platform), ttt["B"], ttt["B"] - ttt["base"]))
    if ttt["B"] <= ttt["base"] + GATE:
        platform.append("B"); best_ttt = min(ttt["B"], ttt["base"]); best_run = "g4_B_x18_20260905"
        log("  관문: B 채택 (목표 구조) → 플랫폼 = %s" % "+".join(platform))
    else:
        diagnose_B("g1_base_x18_20260905", "g4_B_x18_20260905", ttt["base"], ttt["B"])
        # g6_B0 (fail branch): B4c(이중계상) 없는 확정 런을 돌린 뒤 정지
        ttt["B0"] = run("g6_B0_x18_20260905", make_config(["RL", "RS", "B0"]))
        log("  TTT RL+RS+B4b(B4c 없음) = %.1f (기준 대비 %+.1f · g4 대비 %+.1f)" % (ttt["B0"], ttt["B0"] - ttt["base"], ttt["B0"] - ttt["B"]))
        ttt["P0B"] = run("g7_P0B_x18_20260905", make_config(["RL", "B0", "P0"]))
        log("  TTT RL+B4b+P0(가격 off) = %.1f (기준 대비 %+.1f · B0 대비 %+.1f)" % (ttt["P0B"], ttt["P0B"] - ttt["base"], ttt["P0B"] - ttt["B0"]))
        ttt["P0"] = run("g8_P0_x18_20260905", make_config(["P0"]))
        log("  TTT 정본+P0(가격 off, B 없음) = %.1f (기준 대비 %+.1f) → B 의 순효과(같은 채점기) = %+.1f" % (ttt["P0"], ttt["P0"] - ttt["base"], ttt["P0B"] - ttt["P0"]))
        # v7.4: 미터 폐쇄의 도시 비용 가시화 — RL+B4b+P0+B5
        ttt["B5"] = run("g9_B5_x18_20260905", make_config(["RL", "B0", "P0", "B5"]))
        log("  TTT RL+B4b+P0+B5 = %.1f (기준 대비 %+.1f · g7(P0B) 대비 %+.1f)" % (ttt["B5"], ttt["B5"] - ttt["base"], ttt["B5"] - ttt.get("P0B", ttt["B5"])))
        # v7.4: B 없는 대조 — RL+P0 (g8 정본+P0 는 RL 이 없어 램프 신호 정련이 무력(drain=0·가격 0)했던 교란 런)
        ttt["RLP0"] = run("g10_RLP0_x18_20260905", make_config(["RL", "P0"]))
        log("  TTT RL+P0(B 없음) = %.1f (기준 대비 %+.1f) → B 의 순효과(같은 채점기 RL+P0) = g7 %+.1f · g9 %+.1f" % (ttt["RLP0"], ttt["RLP0"] - ttt["base"], ttt.get("P0B", ttt["RLP0"]) - ttt["RLP0"], ttt["B5"] - ttt["RLP0"]))
        log("=== B 진단 정지 · TTT %s ===" % json.dumps({k: round(v, 1) for k, v in ttt.items()}))
        return
    # ③ uni1800
    ttt["uni"] = run("g5_uni1800_x18_20260905", make_config(platform + ["uni1800"]))
    log("  TTT uni1800(%s 위) = %.1f (플랫폼 대비 %+.1f)" % ("+".join(platform), ttt["uni"], ttt["uni"] - ttt["B"]))
    if ttt["uni"] <= ttt["B"] - GATE:
        platform.append("uni1800"); best_ttt = ttt["uni"]; best_run = "g5_uni1800_x18_20260905"
        log("  관문: uni1800 채택 → 플랫폼 = %s" % "+".join(platform))
    else:
        log("  관문: uni1800 기각 → 플랫폼 = %s" % "+".join(platform))
    # v7.1: B4c(이중계상) 없는 확정 런 — RL+RS+B4b. B 의 진짜 효과 판정은 이 값으로.
    ttt["B0"] = run("g6_B0_x18_20260905", make_config(["RL", "RS", "B0"]))
    log("  TTT RL+RS+B4b(B4c 없음) = %.1f (기준 대비 %+.1f · g4(B4c 포함) 대비 %+.1f)" % (ttt["B0"], ttt["B0"] - ttt["base"], ttt["B0"] - ttt["B"]))
    # v7.2: 현시가격 자체를 끈 팔 둘 — 귀속을 위해 B 유무 양쪽. (t=2700 오프라인: 가중 0 이면 SC1002 22/23/71/23 · SC1001 25/20/68/25 · SC101 28/23/65/23)
    ttt["P0B"] = run("g7_P0B_x18_20260905", make_config(["RL", "B0", "P0"]))
    log("  TTT RL+B4b+P0(가격 off) = %.1f (기준 대비 %+.1f · B0 대비 %+.1f)" % (ttt["P0B"], ttt["P0B"] - ttt["base"], ttt["P0B"] - ttt["B0"]))
    ttt["P0"] = run("g8_P0_x18_20260905", make_config(["P0"]))
    log("  TTT 정본+P0(가격 off, B 없음) = %.1f (기준 대비 %+.1f) → B 의 순효과(같은 채점기) = %+.1f" % (ttt["P0"], ttt["P0"] - ttt["base"], ttt["P0B"] - ttt["P0"]))
    ...
    ttt["RLP0"] = run("g10_RLP0_x18_20260905", make_config(["RL", "P0"]))
    log("  TTT RL+P0(B 없음) = %.1f (기준 대비 %+.1f) → B 의 순효과(같은 채점기 RL+P0) = g7 %+.1f · g9 %+.1f" % (ttt["RLP0"], ttt["RLP0"] - ttt["base"], ttt.get("P0B", ttt["RLP0"]) - ttt["RLP0"], ttt["B5"] - ttt["RLP0"]))
    log("=== 자동 구간 끝. TTT %s ===" % json.dumps({k: round(v, 1) for k, v in ttt.items()}))
    assess(platform, best_run, best_ttt)
