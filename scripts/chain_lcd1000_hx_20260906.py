...
def main():
    prefix = sys.argv[1] if len(sys.argv) > 1 else "h6"
    tags = sys.argv[2:] or ["METER", "SAT", "SAT2", "SAT3", "CAP", "QB"]
    cfg_name = make_config(tags)
    name = "%s_%s_lcd1000_x18_20260906" % (prefix, "_".join(t.lower() for t in tags))
    log("=== lcd1000 %s 시작 · %s (%s) · 기준 h0 8082.3 / h1 8629.0 / h3 8469.3 / h5 8652.1 ===" % (prefix, "+".join(tags), cfg_name))
    ttt = run(name, cfg_name)
    ref = {}
    for nm in ("h3_meter_lcd1000_x18_20260906", "h5_meter_sat_sat2_lcd1000_x18_20260906", "h6_meter_sat_sat2_sat3_cap_qb_lcd1000_x18_20260906"):
        try:
            ref[nm] = ttt_of(nm)
        except Exception:
            pass
    log("  TTT %s(%s) = %.1f (무제어 대비 %+.1f · h1 대비 %+.1f%s)" % (prefix, "+".join(tags), ttt, ttt - 8082.3, ttt - 8629.0, "".join(" · %s 대비 %+.1f" % (k[:8], ttt - v) for k, v in ref.items())))
    log("  -- SC1002 녹색(p1/p2/p3/p4) 궤적")
    for nm in ("h5_meter_sat_sat2_lcd1000_x18_20260906", name):
        log("     %s: %s" % (nm[:8], p4_series(nm)))
    log("  -- 링크 329 실제 방류")
    for nm in ("h0_nocontrol_lcd1000_x18_20260906", "h5_meter_sat_sat2_lcd1000_x18_20260906", name):
