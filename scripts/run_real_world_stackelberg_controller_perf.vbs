...
' ---------------------------------------------------------------------------
' PERF INSTRUMENTATION (measurement only). Every perf hook is a timer read, a
' dictionary increment or an Echo. Nothing here touches VISSIM state, so the
' actuation trajectory of this script is identical to the uninstrumented one.
' Enable with RW_PERF=1 in the process environment.
' ---------------------------------------------------------------------------
Dim perfEnabled, perfT, perfN, perfRunT0
Dim perfLastDsd, perfLastSig, perfLastRamp
Set perfT = CreateObject("Scripting.Dictionary")
Set perfN = CreateObject("Scripting.Dictionary")
Set perfLastDsd = CreateObject("Scripting.Dictionary")
Set perfLastSig = CreateObject("Scripting.Dictionary")
Set perfLastRamp = CreateObject("Scripting.Dictionary")
perfEnabled = PerfRequested()
perfRunT0 = Timer
...
Sub RunStepwiseMode()
    Dim pt
    pt = Timer : Vissim.Simulation.RunSingleStep : PerfAdd "sim.run_single_step", pt
    WScript.Echo "RUN_SINGLE_STEP sim_sec=1"
    InitializeComRampMeterControl
    pt = Timer : RunControllerDecision 1 : PerfAdd "decision.total", pt
    pt = Timer : ApplyRuntimeSignals 1 : PerfAdd "apply.runtime_signals", pt
    pt = Timer : ApplyRuntimeRampMeters 1 : PerfAdd "apply.runtime_ramp_meters", pt
    pt = Timer : ApplyIncidentLaneClosure 1 : PerfAdd "apply.incident", pt
    pt = Timer : LogStateCsv 1 : PerfAdd "log.state_csv", pt

    Dim stepNo
    For stepNo = 2 To CLng(simPeriod)
        If stepNo Mod CLng(controlInterval) = 0 Then
            pt = Timer : RunControllerDecision stepNo : PerfAdd "decision.total", pt
        End If
        pt = Timer : ApplyRuntimeSignals stepNo : PerfAdd "apply.runtime_signals", pt
        pt = Timer : ApplyRuntimeRampMeters stepNo : PerfAdd "apply.runtime_ramp_meters", pt
        pt = Timer : ApplyIncidentLaneClosure stepNo : PerfAdd "apply.incident", pt
        pt = Timer : Vissim.Simulation.RunSingleStep : PerfAdd "sim.run_single_step", pt
        ...
        If stepNo Mod CLng(stateLogIntervalSec) = 0 Or stepNo = CLng(simPeriod) Then
            pt = Timer : LogStateCsv stepNo : PerfAdd "log.state_csv", pt
        End If
    Next
End Sub
...
Sub RunEventContinuousMode()
    ...
    Do While CLng(currentSec) < CLng(simPeriod)
        ...
        Dim pt2
        pt2 = Timer : RunContinuousTo CLng(targetSec) : PerfAdd "sim.run_continuous", pt2
        currentSec = CLng(targetSec)
        ...
        If dueToControlStart Then
            If dueToLog Then
                pt2 = Timer : LogStateCsv CLng(currentSec) : PerfAdd "log.state_csv", pt2
                loggedAtCurrentSec = True
            End If
            pt2 = Timer : RunControllerDecision CLng(currentSec) : PerfAdd "decision.total", pt2
            mainControlApplied = True
        ElseIf dueToRepeatedControl Then
            pt2 = Timer : RunControllerDecision CLng(currentSec) : PerfAdd "decision.total", pt2
        End If
        pt2 = Timer : ApplyRuntimeSignals CLng(currentSec) : PerfAdd "apply.runtime_signals", pt2
        pt2 = Timer : ApplyRuntimeRampMeters CLng(currentSec) : PerfAdd "apply.runtime_ramp_meters", pt2
        pt2 = Timer : ApplyIncidentLaneClosure CLng(currentSec) : PerfAdd "apply.incident", pt2
...
Sub RunControllerDecision(simSec)
    ...
    pt = Timer : WriteStateJson simSec, stateJsonPath : PerfAdd "decision.write_state_json", pt
    ...
    PerfAdd "decision.python_subprocess", wallT0
    ...
    Else
        ...
        pt = Timer : ApplyActionCsv simSec, outCsvPath : PerfAdd "decision.apply_action_csv", pt
        ...
        ElseIf Trim(line) <> "" Then
            ...
            If UBound(parts) >= 12 Then
                ...
                If kind = "vsl" Then
                    ...
                    PerfTrackChange "dsd", dsdNo, CLng(speed)
                    pt = Timer
                    pt2 = Timer
                    Set dsd = Vissim.Net.DesSpeedDecisions.ItemByKey(dsdNo)
                    PerfAdd "com.dsd_itembykey", pt2
                    pt2 = Timer
...
Sub ApplyRuntimeSignalController(scNo, majorState, minorState)
    ...
    End If
    On Error GoTo 0
    PerfAddSpan "com.sc_itembykey", ta, tb
    ta = Timer
    sgCount = SignalGroupCount(sc)
    PerfAdd "com.sgs_count", ta
    For sgNo = 1 To sgCount
        On Error Resume Next
        ta = Timer
        Set sg = sc.SGs.ItemByKey(CLng(sgNo))
        ...
        Else
            ...
            End If
        End If
        PerfAddSpan "com.sg_itembykey", ta, tb
    Next
End Sub
...
Function SetSignalGroupState(scNo, sgNo, state)
    Dim sc, sg
    ' Perf timestamps are taken into locals only. No procedure call happens
    ' between the COM write and the Err.Number test, so the Err state the
    ' original code sees is untouched.
    Dim ta, tb, tc, td, te
    SetSignalGroupState = ""
    On Error Resume Next
    ta = Timer
    ...
    te = Timer
    On Error GoTo 0
    PerfTrackChange "sig", "sigstate_" & CStr(scNo) & "-" & CStr(sgNo), CStr(state)
    PerfAddSpan "com.sc_itembykey", ta, tb
    PerfAddSpan "com.sg_itembykey", tb, tc
    PerfAddSpan "com.set_sigstate", tc, td
    PerfAddSpan "com.get_sigstate_readback", td, te
End Function
...
Sub WriteStateJson(simSec, path)
    ...
    ts.WriteLine "}"
    ts.Close
    PerfAdd "state_json.file_write", ptw
End Sub
...
Sub LogStateCsv(simSec)
    ...
    ptl = Timer
    ComputeDetailedState total, urban, freeway, ramp, boundary, other, meanSpeed, freewayMeanSpeed, stopped, countE, speedE, countW, speedW
    PerfAdd "log.compute_detailed_state", ptl
    ...
    PerfAdd "log.bottleneck_total", ptl
End Sub
...
Sub LogBottleneckCsv(simSec)
    ...
    Dim linkNo, key, pos, speed, seg, chainPos
    ...
    PerfAdd "scan.bottleneck_vbs_loop", ptb
    ptb = Timer
    ...
    PerfAdd "log.bottleneck_csv_write", ptb
End Sub
...
Sub ComputeDetailedState(ByRef total, ByRef urban, ByRef freeway, ByRef ramp, ByRef boundary, ByRef other, ByRef meanSpeed, ByRef freewayMeanSpeed, ByRef stopped, ByRef countE, ByRef speedE, ByRef countW, ByRef speedW)
    ...
    Dim laneArray, posArray, speedArray, ok, lo, hi, row
    Dim linkNo, pos, speed, speedSum, freewaySpeedSum, seg, chainPos
    Dim ta
    ...
    lo = MultiLBound(laneArray)
    hi = MultiUBound(laneArray)
    PerfCountBy "scan.compute_detailed_rows", (hi - lo + 1)
    For row = lo To hi
        total = total + 1
        linkNo = FirstInt(MultiValue(laneArray, row))
        pos = CDblOrZero(MultiValue(posArray, row))
        ...
        End If
    Next
    PerfAdd "scan.compute_detailed_vbs_loop", ta
...
Function PerfRequested()
    ...
    v = LCase(CStr(sh.Environment("PROCESS")("RW_PERF")))
    If Err.Number = 0 Then PerfRequested = (v = "1" Or v = "true" Or v = "yes")
    Err.Clear
    On Error GoTo 0
End Function

Sub PerfCount(key)
    If Not perfEnabled Then Exit Sub
    If Not perfN.Exists(CStr(key)) Then perfN.Add CStr(key), 0.0
    perfN(CStr(key)) = CDbl(perfN(CStr(key))) + 1.0
End Sub

Sub PerfCountBy(key, n)
    If Not perfEnabled Then Exit Sub
    If Not perfN.Exists(CStr(key)) Then perfN.Add CStr(key), 0.0
    perfN(CStr(key)) = CDbl(perfN(CStr(key))) + CDbl(n)
End Sub
...
Sub PerfAdd(key, t0)
    ...
    d = Timer - CDbl(t0)
    If d < 0 Then d = d + 86400.0
    If Not perfT.Exists(CStr(key)) Then perfT.Add CStr(key), 0.0
    perfT(CStr(key)) = CDbl(perfT(CStr(key))) + d
    PerfCount CStr(key)
End Sub

Sub PerfAddSpan(key, t0, t1)
    Dim d
    If Not perfEnabled Then Exit Sub
    d = CDbl(t1) - CDbl(t0)
    If d < 0 Then d = d + 86400.0
...
' Change-rate probe: did the value written under this key differ from the
' previous write? Pure bookkeeping - the caller writes unconditionally either
' way, so the plant sees the same sequence of COM writes.
Sub PerfTrackChange(bucket, key, value)
    Dim store, full
    If Not perfEnabled Then Exit Sub
    Select Case CStr(bucket)
        Case "dsd": Set store = perfLastDsd
        Case "sig": Set store = perfLastSig
        Case Else: Set store = perfLastRamp
    End Select
    full = CStr(key)
    If store.Exists(full) Then
        If CStr(store(full)) = CStr(value) Then
            PerfCount "change." & bucket & ".same"
        Else
            PerfCount "change." & bucket & ".diff"
        End If
    Else
        PerfCount "change." & bucket & ".first"
    End If
    store(full) = CStr(value)
End Sub

Sub PerfReport()
    Dim k
    If Not perfEnabled Then Exit Sub
    WScript.Echo "PERF_TOTAL_WALL_SEC=" & Num(ElapsedSec(perfRunT0))
    For Each k In perfT.Keys
        WScript.Echo "PERF=" & k & " sec=" & Num(perfT(k)) & " n=" & CStr(CLng(perfN(k)))
    Next
    For Each k In perfN.Keys
        If Not perfT.Exists(CStr(k)) Then
            WScript.Echo "PERFCOUNT=" & k & " n=" & CStr(CLng(perfN(k)))
        End If
    Next
End Sub
...
Function ReadVehicleLanePos(ByRef laneArray, ByRef posArray)
    ...
    If Err.Number <> 0 Then
        ...
        Err.Clear
        On Error GoTo 0
        PerfAddSpan "com.getmultiatt_2", ta, tb
        PerfCountBy "com.getmultiattvalues_calls", 2
        Exit Function
    End If
    On Error GoTo 0
    PerfAddSpan "com.getmultiatt_2", ta, tb
...
Function ReadVehicleLanePosSpeed(ByRef laneArray, ByRef posArray, ByRef speedArray)
    ...
    If Err.Number <> 0 Then
        ...
        Err.Clear
        On Error GoTo 0
        PerfAddSpan "com.getmultiatt_3", ta, tb
        PerfCountBy "com.getmultiattvalues_calls", 3
        Exit Function
    End If
    On Error GoTo 0
    PerfAddSpan "com.getmultiatt_3", ta, tb
