...
If WScript.Arguments.Count < 3 Then
    WScript.Echo "Usage: cscript run_com_speed_controller.vbs <network.inpx> <state_output.csv> <action_output.csv> [sim_period_sec] [urban_volume_vph] [freeway_volume_vph] [control_interval_sec] [rand_seed] [generated_config.vbs]"
    WScript.Quit 2
End If

Dim netPath, stateOutPath, actionOutPath, simPeriod, urbanVol, freewayVol, controlInterval, randSeed, generatedConfigPath
...
randSeed = ArgOrDefault(7, 7)
generatedConfigPath = ArgOrDefaultText(8, "")
...
Const MODE_NAME = "COM_VSL_STEP_TEST"
...
stateFile.WriteLine "sim_sec,total_vehicles,urban_vehicles,freeway_vehicles,ramp_vehicles,boundary_vehicles,other_vehicles,mean_speed_kph,freeway_mean_speed_kph,stopped_vehicles,controller_mode,vsl_profile,pre_limit_kph,bot_limit_kph"
actionFile.WriteLine "sim_sec,controller_mode,dsd_no,section,link,lane,set_speed_kph,car_read_kph,hgv_read_kph,bus_read_kph"
...
' Apply the initial VSL before the first vehicles enter.
ApplySpeedController 0, True
...
For stepNo = 2 To CLng(simPeriod)
    ...
    ApplySpeedController stepNo, shouldLogAction
    Vissim.Simulation.RunSingleStep
...
Sub ApplySpeedController(simSec, writeActions)
    ApplyVslSection simSec, writeActions, "EB_PRE_D", "1,2", 33
    ApplyVslSection simSec, writeActions, "EB_D_BOT", "3,4", 33
    ApplyVslSection simSec, writeActions, "EB_PRE_F", "5,6", 33
    ApplyVslSection simSec, writeActions, "EB_F_BOT", "7,8", 33
    ApplyVslSection simSec, writeActions, "WB_PRE_F", "9,10", 34
    ApplyVslSection simSec, writeActions, "WB_F_BOT", "11,12", 34
    ApplyVslSection simSec, writeActions, "WB_PRE_D", "13,14", 34
    ApplyVslSection simSec, writeActions, "WB_D_BOT", "15,16", 34
End Sub

Sub ApplyVslSection(simSec, writeActions, section, dsdNosCsv, linkNo)
    Dim speedKph, parts, dsdNo, dsd, laneText, laneNo
    speedKph = DesiredSpeedForSection(simSec, section)
    ...
    For Each dsdNo In parts
        ...
        If writeActions Then
            ...
            actionFile.WriteLine CStr(simSec) & "," & MODE_NAME & "," & CStr(dsdNo) & "," & section & "," & _
...
Function DesiredSpeedForSection(simSec, section)
    Dim profile
    profile = VslProfile(simSec)
    If profile = "FREE_FLOW" Then
        DesiredSpeedForSection = 120
    ElseIf profile = "MODERATE" Then
        If InStr(1, section, "_BOT", vbTextCompare) > 0 Then
            DesiredSpeedForSection = 80
        Else
            DesiredSpeedForSection = 100
        End If
    Else
        If InStr(1, section, "_BOT", vbTextCompare) > 0 Then
            DesiredSpeedForSection = 60
        Else
            DesiredSpeedForSection = 80
        End If
    End If
...
Sub LogState(simSec)
    ...
    stateFile.WriteLine CStr(simSec) & "," & CStr(total) & "," & CStr(urban) & "," & CStr(freeway) & "," & _
        ...
        CStr(VslPreLimit(simSec)) & "," & CStr(VslBotLimit(simSec))
End Sub
