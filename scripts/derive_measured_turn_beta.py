"""[폐기 2026-08-24] 이 유도는 틀렸다. scripts/derive_routing_turn_beta.py 를 써라.

**정의가 어긋난다.** 모델의 beta 는 링크 끝에 **도착**한 차량의 수요 배분인데
(urban_queue_model.approach_routing -> `queue[m] += beta * arrived`),
`.knr` 은 교차로를 **통과 완료**한 차량, 즉 그 방향에 준 녹색의 **결과**다.
컨트롤러가 최적화하는 변수에서 그 입력을 역산하는 자기실현 루프가 된다.

실측 피해:
  - 미매칭 2,758/44,746 = 6.16% (아래 원 docstring 의 "미매칭 0" 은 SC5 한 접근로 값이다)
  - 매칭 실패분을 OUT 한 덩어리로 모아 균등 분할 -> 없애려던 1/n 이 되살아난다
  - 244개 중 85개가 정확히 0, 그 중 internal 25개(우회전 17 · 직진 2).
    beta=0 은 도착 주입을 영구히 0 으로 만들어 그 현시를 굶긴다.
  - 실런 beta_20260824(sim 2640 에서 중단): 녹색 하한 부착 6.7% -> 9.1%

대조 목적으로만 남긴다. 어댑터에서 `urban.beta.source: "knr"` 로 부를 수 있다.
"""
