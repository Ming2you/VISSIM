...
def uncovered_signal_groups(
    ...
    """
    # **미드블록은 의도적 제외다.** 이 사본은 현시 그룹을 본선(1-8)으로만 만들고
    # 미드블록은 VISSIM native 프로그램 그대로 둔다 - 러너가 `RW_MAINLINE_SG_ONLY=1`
    # 로 이미 그렇게 몰고 있다(VBS:1827-1833). native 녹색이 있는데 창이 없는 것이
    # 정상 상태이므로 "떨어뜨린 SG" 로 세지 않는다. 본선에서 같은 일이 생기면 그것은
    # 여전히 실패다 - 그 검사는 그대로 남는다.
    return [
        f"{node}:{sg_no}"
        for sg_no, count in sorted(window_counts.items())
        if int(count) == 0
        and float(native_green_sec.get(sg_no, 0.0)) > 0.0
        and str(sg_no).isdigit()
        and int(str(sg_no)) <= MAINLINE_SG_MAX
    ]
...
# 본선 SG 상한. 이 망의 규약은 **SG 1~8 이 본선 교차로, 9 이상은 미드블록**이다
# (러너 원문 run_real_world_stackelberg_controller.vbs:1815 이하).
MAINLINE_SG_MAX = 8


def _mainline_phase_groups(phase_signal_groups):
    """현시 그룹에서 미드블록 SG 를 뺀다.

    ## 왜

    지금 계획은 미드블록 SG 를 모델 현시에 함께 넣는다. SC5 p1 = ["4","8","12","16","20","24"].
    그래서 `axis_green_sec["p1"]` 이 **97**(SG20 의 native 창 [50,147])이 되고, 본선 SG4·SG8
    이 native 로 [78,121] 43초라 계획이 그 창을 **43/97 = 0.4433** 분율로 적는다.

    결과: 컨트롤러가 p1 에 54.5초를 주문하면 러너가 본선에 **24.2초만** 배달한다
    (1초 되읽기 12주기 실측으로 확인). 컨트롤러의 레버가 자기도 모르게 0.44 배로 감쇠된다.

    본선만 보면 이미 완전한 4현시 순차다 — SC5:

        p1{4,8}=43  p2{3,7}=23  p3{2,6}=47  p4{1,5}=25   합 138 = 모델 예산
        + 4 x 3s 황색 = 150 = 주기

    ## 왜 안전한가

    러너는 `RW_MAINLINE_SG_ONLY=1` 일 때 `IsControlledSignalGroup(sg)=(sg<=8)` 이고
    미드블록엔 ContrByCOM 조차 안 건다 — VISSIM 자체 프로그램이 그대로 돈다(VBS:1820-1833).
    `ParseSignalGroupPlanConfig`(:1851-1852)가 미드블록을 `sgPlanGroups` 에서 빼고
    `sgPlanMidblockSkips` 로만 센다. 즉 **러너는 이미 본선만 몰고 있고**, 계획만 아직
    미드블록을 현시에 넣어 본선 분율을 왜곡하고 있었다.

    미드블록은 `signal_group_ids`(=EXPECTED 계약)에는 그대로 남으므로
    `ValidateSignalGroupPlanCoverage`(:2000-2035)는 통과한다. 어느 현시에도 없으니
    `window_counts` 가 0 이 되고, 상충 판정의 `active` 필터가 `window_counts > 0` 이라
    (signal_group_plan.py:215-217) 미드블록 상충 쌍은 자동으로 빠진다.

    상충을 잃는 것이 문제가 아닌 이유: 그 "상충" 은 기하도 intergreen 행렬도 아니고
    "native 에서 한 번도 안 겹쳤다" 는 **추론**이다(signal_group_plan.py:222-228).
    본선과 미드블록은 **다른 교차로**라 동시녹색이 나도 무방하다(2026-08-25 사용자 판정).
    """
    out = {}
    for phase, ids in (phase_signal_groups or {}).items():
        kept = [
            str(v) for v in (ids or [])
            if str(v).isdigit() and int(str(v)) <= MAINLINE_SG_MAX
        ]
        out[phase] = kept
    return out
...
def derive(
    ...
    for row in signals:
        ...
        plan = signal_group_plan.build_node_plan(
            ...
            # **본선만.** 미드블록 SG 는 현시에서 빼고 native 로 둔다 — 위 docstring 참조.
            phase_signal_groups=_mainline_phase_groups(entry.get("phase_signal_groups") or {}),
        ...
        payload["midblock_native_signal_groups"] = [
            sg_no for sg_no in sg_ids
            if str(sg_no).isdigit() and int(str(sg_no)) > MAINLINE_SG_MAX
            and float(payload["native_green_sec"].get(sg_no, 0.0)) > 0.0
        ]
        never_green.extend(
            f"{node}:{sg_no}"
            for sg_no in sg_ids
