<#
2026-09-01. 램프 병목 망의 **무제어 기준선**.

2x3 여섯 팔 중 넷이 rampbn 망(lnChgDist 200 + relFlow 17경로)에서 도는데, 그 망은
도시부 흐름 분포가 바뀌어 ncsweep_x18(6692.0)을 기준선으로 쓸 수 없다. 이 런이 그 기준선이다.

**-ForceStepwise 필수** — 없으면 결정 시점 상태 JSON 이 1개만 나와 세그먼트 밀도를 못 본다.

볼 것: 셀별 임계 초과율(특히 FW_W|2 — 산술로는 상류 6466 + 램프 752 = 7218 로 용량 6937 을
104% 넘는데 원본 망에서 초과가 23.5% 뿐이었다) · 램프 커넥터 실측 유량(.fzp) · 합류 셀 밀도.
...
$arms = @(
  @{ name = "nc_rampbn_x18_20260901"; tuning = "evaluation/configs/canon_nolencap_20260828.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn.inpx"; note = "무제어 · 램프 병목 망 x1.8 — 이 망의 기준선" }
)
...
foreach ($arm in $arms) {
  ...
  & $runner -Name $name -OutDir $outDir `
      ...
      -Controller "no-control" -SimPeriod 5400 -ControlIntervalSec 150 -StateLogIntervalSec 30 -Seed $Seed -StallSec 86400 -MaxAttempts 2 -ForceStepwise
