<#
2026-09-01. 2x3 설계 — delta_merge {0, 4} x {far 실측, 램프 병목 망, 둘 다}.

축 1  delta_merge 0.0 / 4.0
      17.3 은 비물리다 — 110 kph 에서 한 스텝(10s)에 53.7 kph 를 깎고 항이 자유류에서
      가장 크다(v/(rho+kappa) 형태라 실제 합류 물리와 부호가 반대). 4.0 은 8.69 kph.
      그리고 17.3 대 4.0 실측이 7602.3 대 7479.8 로 거의 같았다 — 이 축은 지배적이지 않다.

축 2  far merge_rate 실측   ramp_capacity 1800 -> 752/1212/938/555 (.fzp 실측)
      far 램프 항이 q^2/(2*merge_rate) 인데 분모가 1.5~3.2배 과대라 벌점이 과소다.
      실측 재계산: N_UF 축 far 폭 2.05% -> **3.46%** (현재 후보 스프레드 0.44% 의 7.9배).
      g_cong·n_crit 은 안 건드린다 — 램프와 무관해 모든 후보에 같은 값이 더해져
      수준만 바뀌고 폭은 그대로다(실측 확인).

      램프 병목 망   lnChgDist 1000->200 + 경로 relFlow 17개.
      FW_W|2 는 상류 6466 + 램프 752 = 7218 로 용량 6937 을 104% 넘는데 임계 초과가
      23.5% 뿐이다 — 1 km 차로변경이 교란을 퍼뜨린 것으로 본다.
      **도시부 분포가 바뀌므로 ncsweep_x18(6692.0)은 이 망의 기준선이 아니다.**

w_ramp_queue 는 6.0 으로 되돌렸다. 0 으로 빼니 램프큐가 249 -> 1544 로 폭주하고
TTT 가 +910 이었다 — 이중계상이지만 유일한 예방 항이었다(팔로워 terminal cost 는
속성조차 없어 영구 False).

기준: canon_fd3sw18 6699.3 · ramgnedyn 6782.7 · wrq0 d17.3 7602.3 · wrq0 d4.0 7479.8
볼 것: 미터링 구속 결정 수 · leader_ramp_queue_veh · 후보 스프레드 · FW_W|2·5 임계 초과율

실측으로 램프 합류가 병목이 안 된다. 원인 후보 둘을 망 하나씩으로 가른다.

  A  nc_lcd200    램프 커넥터 8개 lnChgDist 1000 -> 200 (VISSIM 기본).
     지금 1000 m 라 합류 교란이 1 km 에 퍼져 세그먼트(1.347 km) 안에서 국소 밀도가 안 오른다.
     **결정적 예측**: FW_W|2 는 상류 6466 + 램프 752 = 7218 로 용량 6937 을 104% 넘는데
     임계 초과가 23.5% 뿐이다. 되돌리면 그 셀이 제대로 막혀야 한다.
     유입·경로·기하 불변 = 단일변수.

  B  nc_rampmax   유입 신설 없이 **경로 relFlow 만**으로 램프 몫을 최대로.
     램프 직결 4결정 + 공급 링크(31·32·68·69)로 보내는 상류 12결정, 총 17경로.
     relFlow 단독은 한계가 있다 — fw14_ramp2 가 램프 relFlow 를 5배 키우고도 램프 유입이
     +9.5%(3378->3700)였다. 1135~1137 은 램프끼리 재배분이고 총량은 공급 링크 통과량이
     정하기 때문이다. 그래서 상류를 같이 올렸다.
     **도시부 흐름 분포가 바뀐다** — 이 망의 무제어가 곧 새 기준선이다.

필요 램프 유입 (합류 셀이 용량 6937 을 넘으려면):
  FW_W|2  471 필요 · 현재 752   이미 충분
  FW_W|5 1681 필요 · 현재 1212  1.4배
  FW_E|3  923 필요 · 현재 555   1.7배
  FW_E|5 2645 필요 · 현재 938   2.8배

볼 것: 셀별 임계 초과율 · 램프 커넥터 실측 유량(.fzp) · 합류 셀 밀도.

**w_ramp_queue 6.0 -> 0.0.** 램프 큐가 여러 겹으로 계상된다 —
  (1) leader.py:916   ramp_queue_penalty = w_ramp_queue x ramp_queue_veh
  (2) metanet.py:716  freeway_ttt += sum(ramp_queue) * dt_h   <- base 에 이미 들어감
  (3) freeway agent own-TTS 의 blocked_q (count_blocked_ramp_inflow=True)
  + freeway_follower.ramp_queue_penalty = 40.0 도 따로 있다
(2)가 base 에 이미 세므로 (1)은 같은 차량을 두 번 센다.
2026-08-30 까지는 램프 큐가 항상 0 이라 이 항이 죽어 있어 안 드러났다. ramp_arrival 교정으로
미터링이 구속하기 시작하자(x1.8 21/37) 큐가 최대 249대까지 쌓이고 이중계상이 살아났다.

**delta_merge 3수준**으로 미터링이 손해인 이유를 가른다.
  17.3  동시적합 최적(홀드아웃 +15.7%)  본선 이득을 크게 봄 -> 많이 조임
   4.0  밀도 무편의점                   중간
   0.0  합류 교란 없음                  이득 경로 없음 -> 안 조임
  (a) delta 과대평가면  17.3 > 4.0 > 0.0 순으로 나빠진다
  (b) 물리적 손해면     delta 와 무관하게 조이는 만큼 나빠진다
  (c) 이중계상만이면    w_ramp_queue=0 만으로 셋 다 회복된다

직전 실측 (w_ramp_queue 6.0 · delta 17.3):
  x1.8  6782.7  기존제어 +83.4   freeway +47.6 · urban +30.5 · 구속 21/37 · 램프큐 249
  x2.2  7117.5  기존제어 +81.8   freeway  +4.9 · urban +56.5 · 구속 18/37 · 램프큐 227

왜. 원본 망(x1.0)에서는 고속도로 레버가 구조적으로 잠긴다 —
  본선 밀도 중앙 8.35 · 최대 25.98   임계 27.0 · **초과 셀-스텝 0.0%**
  본선 유량 약 5000 대 용량 6937 = 72% · 평균속도 102.7 kph
  leader_search_density_stress 전 37결정 0.0000

그리고 leader.py:321 이 그 상태를 명시적으로 처리한다 —
    if density_ratio <= metering_activation_density_ratio (0.95):
        feasible_nuf = max(feasible_nuf, total_ramp_capacity)   # 7200 강제 개방
리더가 못 조이는 게 아니라 **조이지 말라고 설계된 상태**다. 실측 미터링 구속 0/37,
N_UF 하한 1440(막지 않음)인데 선택 중앙 6708.

램프 수요 부족은 원인이 아니다 — 램프가 본선 통행의 19~31% 를 나르고 도착 2738 vph.
본선에 여유가 충분해서 그 유입이 문제를 안 일으킬 뿐이다.

무제어 실측으로 x1.8 임계초과 23.6% · x2.2 31.2% 이고 VSL 도 39~49% 발동한다.
거기서 density_stress 가 비영이 되어 강제 개방이 안 걸린다.

x1.0 사다리: ramparr_gnedyn 4653.5(신기록, sigma 50.7 아래) · fdfit3 4657.8 ·
mergedyn 4662.3 · mergefix 4685.5 · ramparr 4693.9 · 무제어 4819.4

  (1) ramp_arrival 교정   도착 1110 -> 2678 vph (실측 3457)
  (2) metering_in_gne     미터링을 GNE 합의 루프 안으로
  (3) 동역학 4항          tau 7 · nu 48 · kappa 25 · delta_merge 17.3
      + 합류셀 보정 R_F_W 4 -> 5   (delta 가 그 상태에서 적합된 값이라 필수)

따로 켠 실측:
  canon_ramparr (1만)           4693.9  freeway -5.2 · urban +42.1 · 미터링 구속 0/34
  canon_fd3sw{18,22}_mgne (2만)  미터링 해가 **비트 동일** — 도착이 1110 이라 볼 게 없었다
  canon_mergedyn (3만)          4662.3  freeway -2.5 · urban +7.4

셋이 서로를 필요로 한다. (1)이 정보를 만들고 (2)가 그 정보가 도시 agent 에 닿을 통로를 열며
(3)이 미터링에 **이득 경로**를 준다 — delta_merge 가 0 이면 램프 유입이 본선 속도를 안
건드려 조일 이유가 없다. 실제로 (1)만 켜니 리더가 램프를 더 **열었다**(6360 -> 7160).

볼 것: 미터링이 도착을 구속한 결정 수 · leader_ramp_queue_veh · freeway/urban TTT 분해 ·
후보 스프레드 · vsl_selected.

**주의** 지금까지 모든 판정이 무제어 5시드 sigma 50.7 아래다(차이 +4.5 ~ +36.2).
이 팔도 그 안에 들면 '구별 안 된다' 까지만 말할 수 있다.

현행은 queue_drain_horizon_sec = 120.0 스칼라 하나를 램프 넷에 다 쓰고 cap 도 900 스칼라다.

  램프     점유중앙  실측유량  필요drain  현행이 내는 값   비
  R_D_W      7.0     752      33.5s        210       0.28
  R_F_W     26.0    1212      77.2s        780       0.64
  R_D_E      8.0     938      30.7s        240       0.26
  R_F_E      1.0     555       6.5s         30       0.05
  합                3457                  1260       0.36

점유 출처를 **코드로 확인하고** 잰 값이다. 어댑터는 detector_mapping 이 아니라
state_json['ramp_counts'] 를 먼저 읽고 비영이면 거기서 끝낸다(매핑은 그게 전부 0 일 때만
타는 폴백). 8/31 에 config 의 detector_mapping_json 을 쓸 거라 추론했다가 교정값을 한 번
틀렸다(154.8/132.2/78.0/129.7). 통과시간은 수요 x1.0~x2.2 에 걸쳐 상수다(변동계수 0~0.119).

왜 중요한가 — 사슬:
  arrival 이 실측의 36% -> 미터링이 도착을 구속한 결정 0/37 -> 램프 저수지가 안 참
  -> freeway agent 의 blocked_q 가 항상 0 -> 미터링의 도시부 비용이 0 으로 보임
  -> 미터링이 공짜 레버 -> 고속도로 모델을 고칠수록 더 조임 -> urban 이 그만큼 나빠짐
실측이 그 마지막 줄을 세 번 보여줬다(freeway/urban):
  x1.0 mergefix -4.3/+32.1 · x1.0 mergedyn -2.5/+7.4 · x1.8 mergefix -4.8/+18.4

볼 것: forecast_ramp_arrival_total_vph · 미터링이 도착을 구속한 결정 수 ·
leader_ramp_queue_veh · freeway/urban TTT 분해.
...
2026-08-31 재발사. 앞선 두 번은 VSL 110 때문에 죽었다 —
  1차 ERROR=ACTION_CSV_CONTRACT  러너 허용목록에 110 이 없어 vsl 행 71개 통째 거부
  2차 ERROR=VSL_COM_WRITE_READBACK  **망에 110 km/h 분포 자체가 없었다**
허용목록은 정책이 아니라 망 자산의 열거였고, 기존 값의 115 도 망에 없는 유령이었다.
이번엔 망에 110 분포를 신설했다(_dsd110.inpx, 거동상 불활성 — 아무도 참조 안 함).
약 9시간을 버렸다.
...
$arms = @(
  @{ name = "nc_rampbn_x18_20260901"; tuning = "evaluation/configs/canon_nolencap_20260828.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn.inpx"; controller = "no-control"; note = "무제어 · 램프 병목 망 — 이 망의 기준선" },
  @{ name = "canon_far_d00_x18_20260901"; tuning = "evaluation/configs/canon_far_d00_x18_20260901.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18.inpx"; controller = "wu-link"; note = "x1.8 · delta 0.0 · w_rq 0 · far 배수율 실측(G·g_fw·merge 전부 링크평가 150초)" },
  @{ name = "canon_bn_d00_x18_20260901"; tuning = "evaluation/configs/canon_bn_d00_x18_20260901.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn.inpx"; controller = "wu-link"; note = "x1.8 · delta 0.0 · w_rq 0 · 램프 병목 망" },
  @{ name = "canon_farbn_d00_x18_20260901"; tuning = "evaluation/configs/canon_farbn_d00_x18_20260901.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn.inpx"; controller = "wu-link"; note = "x1.8 · delta 0.0 · w_rq 0 · far 실측 + 램프 병목 망" },
  @{ name = "canon_far_d40_x18_20260901"; tuning = "evaluation/configs/canon_far_d40_x18_20260901.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18.inpx"; controller = "wu-link"; note = "x1.8 · delta 4.0 · w_rq 0 · far 배수율 실측(G·g_fw·merge 전부 링크평가 150초)" },
  @{ name = "canon_bn_d40_x18_20260901"; tuning = "evaluation/configs/canon_bn_d40_x18_20260901.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn.inpx"; controller = "wu-link"; note = "x1.8 · delta 4.0 · w_rq 0 · 램프 병목 망" },
  @{ name = "canon_farbn_d40_x18_20260901"; tuning = "evaluation/configs/canon_farbn_d40_x18_20260901.json"; network = "network/real_world_gaepo_modi/modi_eval_userfix_20260814e_fwsweep_x18_rampbn.inpx"; controller = "wu-link"; note = "x1.8 · delta 4.0 · w_rq 0 · far 실측 + 램프 병목 망" }
)
