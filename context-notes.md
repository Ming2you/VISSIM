...
## N4-0 작업1 — `.sig` 재작성에서 내린 결정 (2026-08-12)

### 왜 배리어를 "합집합"으로 재는가

SG 녹색을 그냥 더하면 SC109 에서 183.5 s 가 나온다 — 주기 170 보다 크다. EBT 가 배리어 안
두 단계에 걸쳐 녹색을 이어가기 때문이다(`G=[0,144)` 안에 WBT→EBL 전이가 들어 있다).
앞 회차 `survey_signal_programs` 의 현시 평균 합이 주기와 안 맞았던 것도 같은 이유다.

배리어에 속한 살아있는 main SG 의 (녹색 ∪ 황색) **합집합** 길이를 재면 이 중복이 사라진다.
실 15 SC 전부에서 합집합이 **한 덩어리**이고 `Bmajor + Bminor + N×3 = native 주기` 가 오차
없이 닫힌다(생산자가 조각 수 != 1 이면 거부한다). 즉 실 프로그램 15개는 전부 정상적인
ring-barrier 이고, 배리어 총량은 실측 가능한 양이다. SC1001 은 69/69 로 사용자 손측정과 같다.

### 현시 수를 등두가 아니라 녹색으로 센 이유

앞 회차는 `signalHead` 개수로 "빈 현시"를 세어 8 SC 가 걸린다고 봤다. 이번 스펙은
"녹색을 받는 SG 가 있는가"다. 두 기준은 다른 것을 잰다 — 등두 0 인 SG 도 `.sig` 는 녹색을
주고 있고(SC1 의 WBL 37 s), 그 현시를 지우면 `.sig` 구조가 실제 신호와 달라진다.
녹색 기준으로 세면 영구적색 20개만 현시를 없애고 결과가 107·108·109 뿐이다.

### SC5(SG 24개)를 어떻게 나눴는가

살아있는 12개 = main 1‑8 전부 + midblock 슬레이브 4개(SG10 EBT·14 WBT·20 SBT·24 NBT).
나머지 12개는 영구적색이다(SG18 은 등두가 1개 있는데도 영구적색 — 원본 상태를 그대로 뒀다).

main 8개는 이름이 가리키는 현시 하나에 그대로 붙는다. midblock 4개는 교차로 접근로가 아니라
링크 중간 신호라서 **여러 현시에 걸쳐 녹색을 유지하는 것이 그 기능**이다. 그래서 부모 SG 의
현시 하나로 접지 않고, native 녹색이 각 현시의 native 창을 **절반 이상 덮는가**로 현시 집합을
구하고 그 집합이 목표 순서 위에서 순환 연속인지 확인한 뒤, 그 구간의 녹색 + 사이 clearance 를
새 녹색으로 준다.

    SG10·SG14 : minor 좌 → major 직진 → major 좌   102 s → 101 s
    SG20·SG24 : major 좌 → minor 직진 → minor 좌   104 s →  97 s

부모 현시가 항상 그 집합 안에 들어 있는지도 검사가 잡는다.

### SC109 — 세 제약이 동시에 성립하지 않는 유일한 지점

`주기 150` ∧ `green_min 20` ∧ `배리어 총량 비례 보존` 이 SC109 에서만 충돌한다.
minor 배리어에 살아있는 현시가 minor 좌 하나뿐이고(NBT·SBT·NBL·WBL 이 영구적색) 그 배리어의
native 총량이 정확히 20 s 다. 170→150 축소는 `20 × 141/161 = 17.516 s` 를 만드는데, 현시가
하나라 **배리어 안에서 재배분할 여지가 없다**. N 을 다시 세도 3 이 최소다 — 세 현시 전부
녹색을 받는 SG 를 실제로 갖고 있다.

생산자 기본값은 이 지점에서 `BuildError` 로 **멈춘다**. `--allow-green-min-borrow` 를 줄 때만
major 배리어에서 2.484 s 를 빌려 minor 를 20 s 로 만들고, 그 편차를 `green_min_borrow_sec`
으로 산출물·표·검사에 남긴다. 배리어 몫이 87.58/12.42 → 85.82/14.18 로 움직인다.
**이것을 조용히 넘긴 것이 아니라 명시적으로 예외 처리한 것이다.**

### 왜 활성 프로그램만 고치는가

15개 전부 inpx `progNo` = 1 이다. prog 2(off_peak)·3(aft_peak)은 러너가 안 쓴다. 손대지 않는 쪽이
원본 구조 보존이라는 요구에 맞고, 검사가 prog 2·3 의 XML 이 바이트 동일한지 확인한다.

`offset` 과 `switchpoint` 도 그대로 뒀다. offset 은 N4-7 삼중 잠금 아래 `intent_only` 라
플랜트에 도달하지 않고, 내부 배치가 통째로 바뀐 마당에 offset 만 재계산하는 것은 근거가 없다.

### 쓰기를 속성 단위 수술로 한 근거

`.sig` 41개 전부에 대해 `ET.parse` → `ET.tostring` + `<?xml version="1.0" encoding="UTF-8"?>` +
CRLF 로 되쓰면 **바이트 동일**하다는 것을 먼저 확인했다. 그래서 재직렬화가 파일을 흐트러뜨리지
않는다. 그 위에서 살아있는 SG 는 이미 `cmd` 가 정확히 2개(녹색 시작·적색 시작)이고 영구적색은
1개이므로, 요소를 추가·삭제하지 않고 **속성값만** 바꾼다. SC1001 의 실제 diff 는 4줄이다.

### 확인 못 한 것

- **VISSIM 을 안 띄웠다.** `<sc checkSum=...>` 를 원본 값 그대로 뒀다. VISSIG 가 로드 시
  checkSum 을 검증하는지 모른다. 검증한다면 15개 전부 거부될 수 있다. 알고리즘을 모르므로
  재계산하지 않았다.
- **새 `.sig` 는 아직 아무 경로에도 안 물려 있다.** `modi_eval_rw_control.inpx` 의
  `supplyFile2` 는 원본 이름을 가리킨 채다(원본 inpx 를 덮지 말라는 규칙). 배선은 다음 작업.
- **모델 주기 동일성 5건은 그대로 빨간불이다.** `evaluation/controllers/plant_cycle.py` 의
  `plant_lost_time_sec()` 은 `.sig` 가 아니라 **러너 VBS 의 clearance × 상수 2** 에서 나온다.
