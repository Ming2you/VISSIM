...
def _freeway_substep_events(state: _mn.TrafficState, control: _mn.ControlAction, demand: _mn.DemandStep, cfg: _mn.ExperimentConfig, offramp_capacity_veh_h: _mn.Dict[str, float] | None=None, ramp_release_veh_h: _mn.Dict[str, float] | None=None, ramp_release_diagnostics: _mn.Dict[str, float] | None=None, update_ramp_queues: bool=True, include_ramp_queue_ttt: bool=True, complete_allocator_scope: bool=True) -> _mn.Tuple[float, _mn.Dict[str, float]]:
    ...
    for link in net.freeway_links:
        ...
        # Branch VSL model (d80faf9): passive command cohorts, created from the
        # rollout's first physical stocks. The v2 lane plant tags each cell with
        # the command of its governing sign in the last applied action
        # (component_vsl_initial_commands, lane_plant_runtime.initialize); absent,
        # every vehicle starts at initial_command as in the branch.
        exposure_spec = (getattr(net, 'component_vsl_transport', {}) or {}).get(link)
        exposure = None
        if exposure_spec is not None:
            if not hasattr(state, '_component_vsl_exposure'):
                state._component_vsl_exposure = {}
            if link not in state._component_vsl_exposure:
                state._component_vsl_exposure[link] = VSLExposure(
                    vehicles, exposure_spec, vsl_max,
                    (getattr(net, 'component_vsl_initial_commands', None) or {}).get(link))
        ...
        for i, rho in enumerate(rho_for_flow):
            ...
            # Commands enter only through the relaxation target V (nu, the
            # capacity-drop test and q = rho v lambda are unchanged).
            v_eff = literature_desired_speed(
                (getattr(net, 'freeway_vsl_fd_response', {}) or {}).get(link),
                cfg, link, i, rho, v_eff, vsl_i, vsl_active_i)
            if exposure is not None:
                v_eff = exposure.target(cfg, link, i, rho, v_eff, vsl_i)
                exposure_out.append(q_out * dt_h)
                exposure_commands.append(vsl_i)
        ...
        if exposure is not None:
            exposure.advance(vehicles, next_vehicle_count, [q * dt_h for q in q_inter], exposure_out,
                             core_in0 * dt_h, [q * dt_h for q in ramp_in_by_link[link]], exposure_commands)
        if literature is not None:
