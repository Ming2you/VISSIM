        ...
        for action in actions:
            ...
            with shared_response_gc_scope(getattr(private.cfg.network, 'control_area_defer_response_gc', False)) as gc_stats, shared_query_runtime_scope():
                if 'sdmpc_prediction_sequence' in action.diagnostics:
                    ...
                    with prediction_scope(action, private.cfg, initial.time_sec, horizon_steps):
        ...
        while futures:
            ...
            for future in ready:
                ...
                try:
                    ...
                    exc.add_note(f'Parallel response candidate index={index}, action_sha256={hashlib.sha256(key).hexdigest()}')
                    raise
...
def validate_actual_decision_hold(follower, state, reference, item, *, callbacks,
    ...
    if 'sdmpc_physical_response_reuse' in item:
        if not getattr(follower.cfg.network, 'sdmpc_options', {}).get('response_np_cache'):
            raise ValueError('Physical response reuse is not enabled')
        from evaluation.controllers.sdmpc_response_cache import validate_binding
        validate_binding(reference, item)
    ...
    sdmpc_policy = getattr(getattr(follower.cfg, 'network', None), 'sdmpc_options', {})
    quantities = shared_quantity_constraints(follower, reference, item['quantities'],
        ...
        np_tolerance_veh=options['np_tolerance_veh'],
        nuf_mode=('cap' if sdmpc_policy.get('budget_caps') else 'equality'),
        target_nuf_veh_h=reference.N_UF_star,
        nuf_tolerance_veh_h=(sdmpc_policy['pfo_cap_options']['nuf_tolerance_veh_h']
            if sdmpc_policy.get('budget_caps')
            else options['nuf_tolerance_veh_h']))
