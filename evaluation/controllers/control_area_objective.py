...
class ModelAreaLedger:
    ...
    def pack_completed_response_records(self):
    ...
    def plan_constraint_interval(self, start_sec, tu_sec, tf_sec, k_fu, k_cf, urban_scopes, *, off_ramps=()):
        ...
        if (len(set(scopes)) != len(scopes) or 'urban_allocator' not in scopes or
                set(scopes) - {'urban_allocator', 'shared_approach', 'sc2001_corridor', 'legsplit_allocator',
                    ...
                    'native_input_generation', 'native_prehead_residual'}):
        ...
        try:
            for f in range(int(k_cf)):
                ...
                for scope in ('ramp_release_query', 'freeway_allocator'):
    ...
    def response(self):
        ...
        if getattr(self, '_packed_response_records', None):
            ...
            for payload in self._packed_response_records:
                for key, rows in pickle.loads(payload).items():
