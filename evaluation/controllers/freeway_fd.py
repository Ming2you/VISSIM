...
class VSLExposure:
    ...
    """
    def __init__(self, stocks, spec, maximum):
        ...
        self.cohorts=[{float(spec['initial_command']):float(n)} for n in stocks]
        self.max_residual=0.

    def target(self, cfg, road, cell, rho, legacy, displayed):
        row=(getattr(cfg.network,'freeway_segment_params',{}) or {})[road][cell]
        maximum=max(cfg.freeway_follower.vsl_set);stock=sum(self.cohorts[cell].values())
        if stock<=1e-12:return legacy
        if all(abs(command-displayed)<1e-12 for command,n in self.cohorts[cell].items() if n>1e-12):
            return legacy
        ...
        total=0.
        for command,n in self.cohorts[cell].items():
            active=command<maximum-.5
            default=min(nominal,(1.+net.alpha_vsl)*command) if active else nominal
            total+=n*literature_desired_speed(spec,cfg,road,cell,rho,default,command,active)
        return total/stock
    ...
    def advance(self, old_n, new_n, internal, total_out, entry, ramps, commands):
        ...
        for i,row in enumerate(self.cohorts):
            if abs(sum(row.values())-old_n[i])>1e-7 or total_out[i]>old_n[i]+1e-7:
        ...
        for i,row in enumerate(self.cohorts):
            ...
            incoming=({float(commands[i]):1.} if i in self.signs else
                      {float(self.spec['initial_command']):1.} if i==0 else shares[i-1])
            for cmd,fraction in incoming.items():dest[cmd]=dest.get(cmd,0.)+amount*fraction
            cmd=float(self.spec['ramp_command']);dest[cmd]=dest.get(cmd,0.)+ramps[i]
            residual=abs(sum(dest.values())-new_n[i]);self.max_residual=max(self.max_residual,residual)
            if residual>1e-7:raise ArithmeticError('Passive VSL cohorts do not sum to physical stock')
            next_rows.append({cmd:n for cmd,n in dest.items() if n>1e-14})
        self.cohorts=next_rows
