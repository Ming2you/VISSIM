...
def install_config_switches(tuning: Mapping[str, Any]) -> dict[str, float]:
    ...
    pairs = (
        ...
        # TAU 팔. `leader`/`network` 절은 데이터클래스로 그대로 넘어가 TypeError 가 나므로
    ...
    # `_lane_delay_correction_enabled` 는 env 만 본다(이 어댑터보다 먼저 만들어졌다).
    # config 를 정본으로 두되 다리는 env 로 놓는다 — mainline_share 와 같은 방식이다.
