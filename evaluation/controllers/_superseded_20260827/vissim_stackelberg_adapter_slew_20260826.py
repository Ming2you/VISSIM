...
def install_offset_arm(controller, tuning: Mapping[str, Any]) -> dict[str, float]:
    """offset 유지 마진을 config 로 연다. `urban.offset.keep_margin` 없으면 no-op.

    ## 왜 — 실측 진단 (2026-08-25)

    offset 레버는 **죽어 있지 않다. 두 번 거부된다.**

        탐색에서 0 아닌 offset 을 고른 슬롯   200 / 561  (35.7%)   mainline_20260825
                                              192 / 561  (34.2%)   allfix_20260825
                                              210 / 561  (37.4%)   sc7native_20260824
        최종 유지                              0  (세 런 전부)
        결정당 평가 횟수                       187~197

    되돌리는 자리는 `wu_faithful_follower.py:4632` 이고, 기준은 :424 의
    `offset_keep_margin = 0.005` 다. 저장소 전체에서 config·어댑터가 이 값을 쓰는 곳이
    **0곳**이고 테스트 3곳만 -1.0 으로 우회한다.

        실측 모델 안 이득 (ttt_off-ttt_on)/ttt_off
            평균 0.036%   최대 0.0966%   최소 -0.041%
            요구 마진 0.5%  ->  최대이득/요구 = 0.193  ->  33/33 전량 기각

    ## 이 스위치가 여는 것은 **관문 하나뿐**이다

    관문 2 는 승격 잠금이다 — 실런 37결정 전부 `offset_writer=intent_only` ·
    `offset_promotion_status=NOT_EVALUATED` 이고, `offset_promotion.py:339` 가
    production/test_only 가 아니면 무조건 0.0 을 쓴다. 이 스위치만 켜면
    `offset_intent_suppressed_signals` 숫자만 바뀌고 **플랜트 TTT 는 비트 동일**이다.
    반드시 `install_optimized_offset_arm` 과 같이 써야 한다.

    ## 워커 경계

    **인스턴스 속성이라 spawn 을 넘는다.** 가격 워커는 컨트롤러를 피클해 받으므로
    `nash_solver.__dict__` 에 실린 값이 그대로 간다 — 모듈 몽키패치가 프로세스 경계를
    못 넘는 함정(CLAUDE.md)에 닿지 않는다.
    """
    section = _mapping(_mapping(tuning.get("urban")).get("offset"))
    raw = section.get("keep_margin")
    if raw is None:
        return {"offset_keep_margin_override": 0.0}
    follower = getattr(controller, "nash_solver", None)
    if follower is None:
        return {"offset_keep_margin_override": 0.0, "offset_keep_margin_no_follower": 1.0}
    before = float(getattr(follower, "offset_keep_margin", 0.005))
    follower.offset_keep_margin = float(_as_float(raw, before))
    return {
        "offset_keep_margin_override": 1.0,
        "offset_keep_margin_before": before,
        "offset_keep_margin_after": float(follower.offset_keep_margin),
    }


def install_optimized_offset_arm(control) -> dict[str, float]:
    """최적화기가 고른 offset 을 **강제 arm 표**에 실어 실제로 쓰이게 한다.

    `urban.offset.commit_optimized` 없으면 no-op(비트 동일).

    ## 왜 이 경로인가 — 그리고 무엇을 늘리고 있는지 밝힌다

    `production` writer 는 config 로 **선언할 수 없다**(offset_promotion.py:278-283 이
    명시적으로 거부한다). 삼중 잠금(d_core · n9_offset_effect · n8_4_runtime)의 증거
    산출물 셋이 **하나도 존재하지 않아서**(2026-08-25 확인) 승격도 열리지 않는다.

    그래서 최적화기 offset 이 플랜트에 닿는 길은 `test_only` + 강제표뿐이다.
    다만 그 모듈은 test_only 를 "**강제 offset arm** 만 낸다 — 최적화기가 고른 offset 이
    아니라 harness 가 지정한 값" 이라고 못박아 뒀다. **이 팔은 그 계약을 늘린다.**
    숨기지 않고 적어 둔다:

      * 이것은 **실험 팔**이다. production 승격 경로는 건드리지 않는다 — 삼중 잠금은
        그대로 닫혀 있고 이 스위치로 열리지 않는다.
      * 진단 `offset_arm_optimized_*` 로 무엇을 실었는지 매 결정 남긴다.
      * 스위치가 없으면 표를 안 실으므로 `guard_forced_arm` 도 종전대로 통과한다.

    ## 왜 그냥 두면 안 되는가

    `guard_forced_arm` 의 docstring 이 이유를 적어 뒀다 — 조용히 0 으로 뭉개면
    "돌긴 돌았는데 레버가 없는" 자료가 나오고, 그 자료가 나중에 **offset 효과 없음**으로
    읽힌다. 지금까지의 연동 A/B 결론이 정확히 그 상태다(커밋된 offset 이 항상 0 이었으므로
    무효 대조군).
    """
    # `write_action_csv` 에는 tuning 인자가 없다. 그래서 모듈 스위치로 읽는다 —
    # `install_config_switches` 가 tuning 을 연 직후 심어 둔 값이다.
    if not _switch("offset_commit_optimized", "RW_OFFSET_COMMIT_OPTIMIZED"):
        return {"offset_arm_optimized_enabled": 0.0}
    offsets = getattr(control, "offsets", None) or {}
    table = {str(k): float(v) for k, v in offsets.items()}
    if not table:
        return {"offset_arm_optimized_enabled": 1.0, "offset_arm_optimized_empty": 1.0}
    control.diagnostics[offset_promotion.FORCED_ARM_TABLE_KEY] = json.dumps(
        table, separators=(",", ":")
    )
    nonzero = sum(1 for v in table.values() if abs(v) > 1.0e-9)
    return {
        "offset_arm_optimized_enabled": 1.0,
        "offset_arm_optimized_signals": float(len(table)),
        "offset_arm_optimized_nonzero": float(nonzero),
        "offset_arm_optimized_max_abs_sec": float(max((abs(v) for v in table.values()), default=0.0)),
    }
...
_SLEW_STATE = {"clamped": 0.0, "moves": 0.0, "raw_max": 0.0, "out_max": 0.0}
# 직전 결정이 **플랜트에 실제로 보낸** offset. main 이 previous 를 읽은 직후 채운다.
# 솔버 내부 iterate 가 아니라 이것이 슬루의 기준점이어야 한다.
_PREV_OFFSETS: dict[str, float] = {}


def _offset_slew_sec() -> float:
    """결정당 offset 변경 허용폭[s]. 0 이하이면 끔(비트 동일). config `urban.offset.slew_sec`."""
    return float(_CFG_SWITCH_NUM.get("offset_slew_sec", 0.0))


def _apply_offset_slew(signal: str, offset: float, prev_offset: float, cycle: float) -> float:
    """커밋 offset 을 **직전 커밋값** 기준 +-S 안으로 묶는다(short-way, 원형).

    ## 왜 — 신뢰영역이 있는데도 반주기 점프가 난다 (2026-08-26 실측)

    팔로워에는 이미 신뢰영역이 있고 켜져 있다. 리더가 매 결정
    `follower.offset_marginal_price_trust_sec = delta_o`(= cycle/8 = 18.75s)를 세우고,
    후보 필터가 `abs(_circ_delta(offset, off_ref)) > trust` 를 거른다
    (wu_faithful_follower.py:1815). 그런데 커밋된 offset 의 결정간 점프를 재면:

        전이 318회 · 중앙 **18.75**(177회) · 그런데 43%가 초과
        37.5(77회) · 56.25(47회) · 75.0(11회)   <- 전부 18.75 의 정확한 정수배

    기준점 `off_ref` 는 `op_offset[s] = previous.offsets[s]`(stackelberg_wu_metered.py:1356)
    에서 오는데, 리더는 **ADMM 반복마다 가격을 다시 계산한다**(:1365 `refresh = force or ...`).
    그래서 기준점이 "직전 커밋" 이 아니라 **그 반복의 현재 iterate** 로 갱신되고, 반복마다
    18.75 씩 걸어가 누적된다. 실측으로 리더 ref 와 직전 커밋이 **540 표본 중 207회(38%)**
    어긋나고 최대 56.25s 차이가 난다.

    ## 그래서 여기서 묶는다

    솔버 내부 반복이 무엇을 하든, **플랜트에 닿는 값**을 직전에 플랜트가 받은 값 기준으로
    묶는다. 실제 신호 컨트롤러의 전이 한계(NTCIP/TS2 의 short-way)가 정확히 이 성질이다 —
    offset 을 바꾸려면 주기를 늘이거나 줄여야 하고, 한 주기에 옮길 수 있는 양에 한계가 있다.

    러너는 `pos = FMod(simSec + offset, cycle)` 로 **즉시 점프**시키므로
    (run_real_world_stackelberg_controller.vbs:1595) 큰 점프는 돌던 녹색을 그대로 자른다.
    주기의 20%(평균 29.8s)를 318회면 offset 값이 옳든 그르든 손해다.

    ## 모델-플랜트 정합에 대하여

    여기서 자른 값이 다음 결정의 `previous` 로 그대로 들어가므로(action JSON 의 offsets 가
    CSV 와 일치함을 확인했다 — 629 표본 중 불일치 4건은 의도한 주기 wrap 뿐), 모델은
    다음 스텝에 잘린 값을 기준으로 다시 푼다. 한 스텝 안에서 모델이 평가한 offset 과 커밋된
    offset 이 달라질 수는 있고 그것은 이 팔이 감수하는 근사다 — 대안은 솔버 내부 반복마다
    기준점을 직전 커밋으로 고정하는 vendor 수정인데, 그것은 별건이다.
    """
    if cycle <= 1.0e-9:
        return offset
    slew = _offset_slew_sec()
    if slew <= 0.0:
        return offset
    half = cycle / 2.0
    delta = ((float(offset) - float(prev_offset) + half) % cycle) - half
    _SLEW_STATE["raw_max"] = max(_SLEW_STATE["raw_max"], abs(delta))
    if abs(delta) > 1.0e-9:
        _SLEW_STATE["moves"] += 1.0
    if abs(delta) <= slew + 1.0e-9:
        _SLEW_STATE["out_max"] = max(_SLEW_STATE["out_max"], abs(delta))
        return offset
    limited = (float(prev_offset) + (slew if delta > 0 else -slew)) % cycle
    _SLEW_STATE["clamped"] += 1.0
    _SLEW_STATE["out_max"] = max(_SLEW_STATE["out_max"], slew)
...
# 수치형 스위치(불리언이 아닌 것). 같은 config 절에서 읽는다.
_CFG_SWITCH_NUM: dict[str, float] = {}
...
def install_config_switches(tuning: Mapping[str, Any]) -> dict[str, float]:
    ...
    pairs = (
        ...
        ("offset_commit_optimized",
         _mapping(urban.get("offset")).get("commit_optimized")),
    )
    out: dict[str, float] = {}
    ...
    _slew = _mapping(urban.get("offset")).get("slew_sec")
    if _slew is not None:
        _CFG_SWITCH_NUM["offset_slew_sec"] = float(_as_float(_slew, 0.0))
        out["cfg_offset_slew_sec"] = _CFG_SWITCH_NUM["offset_slew_sec"]
...
def build_local_observation_summary(
    ...
    projection_diagnostics = {
        ...
        # 중첩 dict 는 metadata 평탄화 루프(스칼라만 통과)가 버린다. 스칼라로 편다.
        **{f"moving_speed_{_k}": float(_v) for _k, _v in _moving_speed_diag.items()},
...
def build_priced_wu_link_controller(cfg, tuning: Mapping[str, Any]):
    ...
    # offset 유지 마진. 절이 없으면 no-op.
    install_offset_arm(controller, tuning)
...
def write_action_csv(
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        if write_signal_rows:
            # 최적화기 offset 을 강제 arm 표로. 절이 없으면 no-op 이라 비트 동일이다.
            _offarm_meta = install_optimized_offset_arm(control)
            if isinstance(csv_metadata, MutableMapping):
                csv_metadata.update(_offarm_meta)
            ...
            for signal_row in _signal_rows_for_mapping(mapping):
                ...
                # **offset 을 그 신호의 플랜 주기로 감는다.**
                #
                # 러너가 `IsFiniteNumberInRange(parts(11), 0.0, cycleSec)` 로 offset 이
                # **그 신호의 주기 안**에 있기를 요구한다(VBS:1354). 넘으면 그 SC 의
                # signal_sg 행이 전부 무효가 되고 창 개수가 0 으로 집계돼
                # `plan_reject=window_count` 로 **CSV 전량이 거부**된다 — 그 구간은
                # 플랜트가 무제어로 돈다(2026-08-25 offarm 1차 시도가 26결정 중 19개를
                # 그렇게 잃었다).
                #
                # 왜 넘는가. offset 후보 격자는 **전역 주기 150** 의 분수인데
                # (150 x 0.875 = 131.25) `native_signal` 이 신호별 주기를 심은 뒤로
                # SC7 은 120, SC16 은 플랜 주기 116(녹색 107 + 3현시 x 3s)이다.
                # 격자가 신호별 주기를 안 읽는다 — 또 같은 양식이다.
                #
                # 감는 것이 **의미상 동일**한 이유: 모델의 `_phase_green_fraction` 이
                # `t0 = (step*T_u - offset) % signal_cycle` 로 이미 신호 주기로 감는다
                # (urban_queue_model.py). 그러니 131.25 는 모델 안에서도 이미
                # 131.25 mod cycle 이고, 여기서 감아도 플랜트가 보는 값이 안 바뀐다.
                _plan_cycle = signal_group_plan.plan_cycle_sec(
                    {_p: phase_green[_p] for _p in signal_group_plan.MODEL_PHASES},
                    RUNNER_CLEARANCE_SEC[0], RUNNER_CLEARANCE_SEC[1],
                )
                # `csv_metadata` 는 이 지점에서 **직렬화된 문자열**이다(_action_csv_metadata).
                # 여기에 진단을 넣으려다 항목 대입으로 죽었다 — 감은 사실은 action JSON 의
                # `offsets`(원값)와 CSV 의 offset 열(감은 값)을 대조하면 사후에 복원된다.
                if _plan_cycle > 1.0e-9 and abs(offset) > 1.0e-9:
                    offset = float(offset) % _plan_cycle
                signal_row_out = {
                    "kind": "signal",
                    "id": signal,
                    "sc_no": sc_no,
...
def main() -> None:
    ...
    _PREV_OFFSETS.clear()
    for _sig, _off in (previous.offsets or {}).items():
        _PREV_OFFSETS[str(_sig)] = float(_off)
    ...
    if hasattr(state, "local_observation_summary"):
        ...
        # TAU 팔의 온디스크 증거. τ 는 TTT 하나로 판정할 수 없다(하류 approach 의 도착
        # 시점을 바꾸므로 방출 회계 전체가 움직인다) — 분포와 위상을 매 결정 남긴다.
        tau_diag = summary.get("tau_diagnostics")
        if isinstance(tau_diag, Mapping):
            metadata["tau_diagnostics"] = dict(tau_diag)
            for key, value in tau_diag.items():
                if isinstance(value, (int, float, bool)):
                    metadata[f"tau_{key}"] = float(value)
    ...
    # ---- 슬루 제한 ----
    # action JSON 이 CSV **보다 먼저** 써지므로 여기서 잘라야 JSON·CSV·다음 결정의
    # `previous` 가 전부 같은 값을 본다. CSV 쓰기 시점에 자르면 JSON 이 안 잘려
    # 다음 스텝의 기준점이 어긋난다(첫 판이 그랬고 레버 diff 가 0 으로 나왔다).
    if _offset_slew_sec() > 0.0:
        for _sig in list((control.offsets or {}).keys()):
            _greens = {
                _p: float(control.green_times.get(f"{_sig}_{_p}", 0.0))
                for _p in signal_group_plan.MODEL_PHASES
            }
            _cyc = signal_group_plan.plan_cycle_sec(
                _greens, RUNNER_CLEARANCE_SEC[0], RUNNER_CLEARANCE_SEC[1]
            )
            if _cyc <= 1.0e-9:
                continue
            control.offsets[_sig] = _apply_offset_slew(
                str(_sig), float(control.offsets[_sig]),
                float(_PREV_OFFSETS.get(str(_sig), 0.0)), float(_cyc),
            )
        metadata["offset_slew_sec"] = float(_offset_slew_sec())
        metadata["offset_slew_moves"] = float(_SLEW_STATE["moves"])
        metadata["offset_slew_clamped"] = float(_SLEW_STATE["clamped"])
        metadata["offset_slew_raw_max"] = float(_SLEW_STATE["raw_max"])
        metadata["offset_slew_out_max"] = float(_SLEW_STATE["out_max"])
