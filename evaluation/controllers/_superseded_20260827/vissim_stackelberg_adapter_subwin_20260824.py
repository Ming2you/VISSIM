...
# 본선 SG 상한. 9 이상은 미드블록이라 러너가 COM 으로 건드리지 않는다.
SUBWINDOW_MAINLINE_SG_MAX = 8


def _subwindow_enabled() -> bool:
    """부분창 배달을 모델에 반영할지. 기본 꺼짐 = 비트 동일."""
    return str(os.environ.get("RW_SUBWINDOW_SERVICE", "")).strip().lower() in {"1", "true", "on"}


def build_subwindow_fraction_table() -> dict[str, float]:
    """(신호, 현시) -> 본선 SG 가 실제로 녹색인 현시 분율.

    **왜 필요한가.** 컨트롤러는 *현시* 녹색을 주문하는데, 러너는 그 현시 안에서 SG 의
    **부분 창**만 켠다(`signal_group_actuation_plan_v3.json` 의 `phase_segments`).
    모델은 현시 녹색 전체가 movement 를 서비스한다고 채점하므로 과대평가가 된다.

    실측(conv, 1초 되읽기, 12주기) — 산술이 소수점까지 닫힌다:

        SC5 p1  주문 54.5s x 0.4433 = 24.2s  =  **실측 24.2 s/cycle**
        SC5 p3  주문 22.7s x 0.4653 = 10.6s  =  **실측 11.0 s/cycle**

    SC5 동서직진은 native 가 47 s/cycle 인데 제어 하에서 11 이다.

    **`native_phase_green.share` 와 다른 양이다.** share 는 "축의 녹색 중 이 movement 의
    몫"이라 SG 를 공유하는 movement 들 사이에서는 1.0 이 맞다. 여기서 필요한 것은
    "주문한 현시 녹색 중 실제 배달되는 몫"이고 그것이 창 분율이다. 둘을 혼동해
    `RW_NARROW_AXIS_SG` 로 share 를 살려 봤지만 263개 중 4개만 깎였고(나머지 170개는
    1.0) 결과는 +53 회귀였다(narrowaxis_20260824 4891.7).

    **미드블록은 뺀다.** SG 9+ 는 현시 전체를 덮어(SC5 p3 의 SG10·14 가 1.0) 분율을
    1.0 으로 뭉갠다. 그리고 러너가 그것들을 COM 으로 안 건드리므로 본선 movement 의
    서비스와 무관하다.
    """
    ...
    for entry in (doc.get("controllers") or {}).values():
        node = str(entry.get("node_id", ""))
        segments = _mapping(entry.get("phase_segments"))
        for pid, windows in segments.items():
            ...
            for window in windows:
                if not isinstance(window, Mapping):
                    continue
                sg = str(window.get("sg_no", ""))
                if not sg.isdigit() or int(sg) > SUBWINDOW_MAINLINE_SG_MAX:
                    continue
                a = _as_float(window.get("start_fraction"), 0.0)
                b = _as_float(window.get("end_fraction"), 0.0)
                if b > a:
                    spans.append((a, b))
            if not spans:
                continue
            # 본선 SG 들의 창 **합집합** 폭. 겹치면 한 번만 센다.
            spans.sort()
            total, cur_a, cur_b = 0.0, spans[0][0], spans[0][1]
            for a, b in spans[1:]:
                if a > cur_b:
                    total += cur_b - cur_a
                    cur_a, cur_b = a, b
                    ...
                    cur_b = max(cur_b, b)
            total += cur_b - cur_a
            table[f"{node}_{pid}"] = min(max(total, 0.0), 1.0)
    return table

_SUBWINDOW_TABLE: dict[str, float] = {}
...
def build_patched_phase_green_fraction(original, schedules, share_table):
    ...
    def patched_phase_green_fraction(control, cfg_arg, spec, urban_step_index=None):
        if str(spec.get("phase", "")):
            ...
            # 부분창 배달. share 와 **곱해지는 별개 인자**다 — share 는 축 녹색 중
            # movement 의 몫이고, 이쪽은 주문한 현시 녹색 중 실제 배달되는 몫이다.
            window = 1.0
            if _subwindow_enabled():
                window = _SUBWINDOW_TABLE.get(str(spec.get("phase", "")), 1.0)
            share = share_table.share_for(spec)
            base = original(control, cfg_arg, spec, urban_step_index) * window
            if share is None:
                return base
            return base * share
        node = str(spec.get("intersection", ""))
...
def install_monitor_fixed_signal_runtime_patch(
    ...
    global _SUBWINDOW_TABLE
    _SUBWINDOW_TABLE = build_subwindow_fraction_table() if _subwindow_enabled() else {}
    ...
    diagnostics: dict[str, Any] = {
        ...
        "subwindow_service_enabled": 1.0 if _subwindow_enabled() else 0.0,
        "subwindow_service_phase_count": float(len(_SUBWINDOW_TABLE)),
        "subwindow_service_min": float(min(_SUBWINDOW_TABLE.values())) if _SUBWINDOW_TABLE else 1.0,
        "subwindow_service_scaled_count": float(
            sum(1 for v in _SUBWINDOW_TABLE.values() if v < 1.0 - 1.0e-9)),
    }
