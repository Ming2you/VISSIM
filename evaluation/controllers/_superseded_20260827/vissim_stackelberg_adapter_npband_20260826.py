...
def install_dual_scale_arm(controller, tuning: Mapping[str, Any]) -> dict[str, float]:
    """λ_P 의 **스케일**을 config 로 연다. `urban.dual` 절이 없으면 no-op(비트 동일).

    ## 왜 — 실측 (2026-08-26)

    듀얼이 가격이 아니라 **스위치**로 동작한다. 팔로워 후보비용은
    `cost = local_TTS + λ_P · nin_i(green)` 인데 두 항의 크기가 안 맞는다:

        Δcost  (후보 최대-최소)   2.7 ~ 12.3  veh·h
        Δnin                     37 ~ 235    veh
        λ_P cap                  10.0        ->  λ·Δnin = 368 ~ 2350

    λ 를 쓸어 본 결과(t=002250, nin 이 움직이는 5신호의 argmin p1[s]):

        신호      Δcost    λ=0   0.005   0.01   0.02   0.05    0.1     10
        SC1        4.18   43.0   43.0   43.0   43.0   43.0   43.0   20.0
        SC108      4.65   39.0   39.0   39.0   34.0   34.0   34.0   20.0
        SC109     12.34   20.0   20.0   20.0   20.0   20.0   20.0  101.0
        SC1001     8.73   20.0   20.0   20.0   23.0   78.0   78.0   78.0
        SC1004     2.74   22.0   24.0   49.0   63.0   78.0   78.0   78.0

    **λ ≈ 0.005~0.02 에서만 내부해가 나온다.** 0.05 이상은 전부 상자 꼭짓점(20 / 78 / 101)
    으로 포화한다. 현행 cap 10 은 그 구간보다 500~2000배 크다.

    갱신식도 같이 어긋나 있다 — `λ_next = clip(λ + gain·(Σnin − target), 0, cap)` 에서
    gain=0.01 이고 잔차가 O(100) veh 라 **한 스텝에 cap 으로 튄다.** 그래서 어떤 N_P
    후보를 골라도 같은 λ(=cap 포화)가 나오고, 같은 팔로워 응답이 나오고, 리더 목적함수가
    평평해진다 — 실측 `leader_candidate_objective_gap / objective = 2.7e-06`.

    이것이 리더 층이 불활성인 이유의 상류다. 정적 N_P 상자를 폭 2650 -> 700 으로 좁혀도
    레버가 0/91 로 안 바뀌었던 것(B_box 팔)이 같은 원인이다 — 상자가 문제가 아니라
    **상자 안 어디를 골라도 λ 가 같아서** 목적함수가 후보를 못 가른다.

    ## 무엇을 여는가

    `lambda_np_cap`(:445) · `lambda_np_step_gain`(:444) · `dual_step_c`(:441) 셋 다
    팔로워 **인스턴스 속성**이고 저장소 전체에서 config·어댑터가 쓰는 곳이 0곳이다.
    인스턴스 속성이라 컨트롤러와 함께 피클되어 **가격 워커 spawn 을 넘는다**(offset
    keep_margin 과 같은 경계).

    ## 주의

    cap 을 낮추면 λ 가 **Σnin 을 target 으로 끌고 가는 힘**도 같이 줄어든다. 목적은
    추종이 아니라 **거래**다 — 지금은 포화라 거래가 없다. 추종 성능은 별도로 봐야 한다
    (`wu_faithful_sum_nin_veh` 대 리더 target 잔차).
    """
    section = _mapping(_mapping(tuning.get("urban")).get("dual"))
    if not section:
        return {"dual_scale_override": 0.0}
    follower = getattr(controller, "nash_solver", None)
    if follower is None:
        return {"dual_scale_override": 0.0, "dual_scale_no_follower": 1.0}
    out: dict[str, float] = {"dual_scale_override": 1.0}
    for key, attr, default in (
        ("lambda_cap", "lambda_np_cap", 10.0),
        ("lambda_gain", "lambda_np_step_gain", 0.01),
        ("dual_step_c", "dual_step_c", 1.0),
    ):
        raw = section.get(key)
        if raw is None:
            continue
        before = float(getattr(follower, attr, default))
        setattr(follower, attr, float(_as_float(raw, before)))
        out[f"dual_{key}_before"] = before
        out[f"dual_{key}_after"] = float(getattr(follower, attr))
    return out
...
def _np_state_band_enabled() -> bool:
    """리더 N_P 후보 상자를 **매 스텝 상태에서** 만들지. config `urban.leader_np.state_band`."""
    return _switch("np_state_band", "RW_NP_STATE_BAND")


def install_np_state_band() -> dict[str, float]:
    """리더의 N_P 후보 상자를 팔로워의 **실현가능 밴드**로 매 결정 다시 만든다.

    ## 왜 — 배선이 빠진 자리 (2026-08-26)

    `leader.py:661` 이 이렇게 생겼다:

        def _np_candidate_bounds(self, state: TrafficState) -> tuple[float, float]:
            # (독스트링) Calibration된 n_P_crit 주변으로 leader의 도시 누적 목표 후보를 제한한다.
    ...
    **`state` 를 받아 놓고 안 쓴다.** 독스트링은 상태로 제한한다고 적어 뒀는데 본문은 정적
    config(`[-250, 2400]`)를 그대로 돌려준다.

    ## 무엇이 틀어지나 — 실런 24결정 실측

        n_p_star = -250.0 (상자 바닥)  ->  pd_target 이 pd_min 으로 클램프
                                       ->  잔차 = Σnin - pd_min > 0  ->  λ 발화 -> cap    14결정
        n_p_star = Σnin 그대로         ->  잔차 ~ 0 -> 조기탈출 ①      ->  λ = 0            9결정

    상자 바닥이 실현가능 최소(535~958)보다 훨씬 아래라 **바닥을 고르면 언제나 같은 클램프**가
    된다. 리더 목적함수가 후보를 못 가르므로(1위-2위/목적함수 = 2.7e-06) 둘 중 어디에 떨어질지도
    사실상 임의다. 상자를 [420, 1120] 으로 좁혀 본 팔이 레버를 0/91 바꾼 것도 이 때문이다 —
    420 도 여전히 밴드 **아래**라 같은 클램프였다.

    ## 무엇으로 바꾸는가

    팔로워가 이미 계산하는 `_np_feasible_range` 를 쓴다. 그것은 신호별로 실제 green 후보집합
    위에서 `served = min(큐 + 도착예측, horizon x green분율 x 용량)` 을 유입에서 유출 빼고
    합산한 것이라, 사용자가 말한 **"유출 대기 큐+approach 합 ~ 유입 대기 큐+approach 합"**
    그 자체다. 지금은 pd_target 클램프에만 쓰이고 후보 생성에는 안 쓰인다.

    이 설치는 `decide_with_info` 를 감싸 (1) 그 밴드를 재고 (2) `cfg.leader.N_P_star_range` 를
    그 값으로 잠시 바꾸고 (3) 원본을 부르고 (4) 되돌린다. `_np_candidate_bounds` 가 호출
    시점에 config 를 새로 읽으므로 이것으로 충분하다 — 리더 코드를 안 건드린다.

    ## 안전장치

    밴드가 뒤집히거나(hi<=lo) 폭이 1 veh 미만이면 **원래 상자를 그대로 둔다.** 상자를 점으로
    만들어 리더를 더 죽이지 않는다. 진단으로 매 결정 밴드와 사용 여부를 남긴다.

    기본 OFF = 비트 동일.
    """
    if not _np_state_band_enabled():
        return {"np_state_band": 0.0}
    if _NP_BAND_STATE.get("installed"):
        return {"np_state_band": 1.0, "np_state_band_reinstall": 1.0}
    from src.controllers.stackelberg_wu_metered import StackelbergWuMeteredController
    from src.models.state import ControlAction

    orig = StackelbergWuMeteredController.decide_with_info
    _NP_BAND_STATE["orig"] = orig

    def _band(self, state, forecast, previous):
        """이 결정에서 팔로워가 실제로 만들 수 있는 Σnin 범위 [veh]."""
        fol = getattr(self, "nash_solver", None)
        if fol is None or not forecast:
            return None
        cfg = self.cfg
        demand = forecast[0]
        horizon_h = float(cfg.mpc.horizon_steps) * float(cfg.simulation.T_c_h)
        base = previous if previous is not None else ControlAction.uncontrolled(cfg)
        ctrl0 = ControlAction.uncontrolled(cfg)
        ctrl0.green_times = dict(base.green_times)
        ctrl0.offsets = dict(base.offsets)
        ctrl0.vsl = dict(base.vsl)
        ctrl0.ramp_metering = dict(base.ramp_metering)
        ctrl0.inflow_outflow_allocation = {}
        coupling = fol._wu._coupling(state, ctrl0, demand)
        snapshot = ControlAction(
            ramp_metering=dict(ctrl0.ramp_metering), vsl=dict(ctrl0.vsl),
            green_times=dict(ctrl0.green_times), offsets=dict(ctrl0.offsets),
            inflow_outflow_allocation={},
        )
        fa = fol._movement_forecast_arrivals_veh(list(forecast))
        return fol._np_feasible_range(state, coupling, snapshot, fa, horizon_h)

    def wrapper(self, state, forecast, previous, cfg=None, *args, **kwargs):
        lc = self.cfg.leader
        saved = list(lc.N_P_star_range)
        meta = {"np_state_band": 1.0, "np_state_band_applied": 0.0}
        try:
            band = _band(self, state, forecast, previous)
        except Exception as exc:  # 조용히 원래 상자로 떨어지지 않는다 — 사유를 남긴다.
            band = None
            meta["np_state_band_error"] = 1.0
            _NP_BAND_STATE["last_error"] = f"{type(exc).__name__}: {exc}"
        if band is not None:
            lo, hi = float(band[0]), float(band[1])
            meta["np_state_band_lo"] = lo
            meta["np_state_band_hi"] = hi
            meta["np_state_band_width"] = hi - lo
            if hi - lo >= 1.0:
                lc.N_P_star_range = [lo, hi]
                meta["np_state_band_applied"] = 1.0
        _NP_BAND_STATE["meta"] = meta
        try:
            if cfg is None:
                return orig(self, state, forecast, previous, *args, **kwargs)
            return orig(self, state, forecast, previous, cfg, *args, **kwargs)
        finally:
            lc.N_P_star_range = saved

    wrapper.__name__ = orig.__name__
    StackelbergWuMeteredController.decide_with_info = wrapper
    _NP_BAND_STATE["installed"] = True
    if StackelbergWuMeteredController.decide_with_info is orig:
        raise RuntimeError("NP-BAND 설치 실패: decide_with_info 가 안 바뀌었다")
    return {"np_state_band": 1.0}
...
def install_config_switches(tuning: Mapping[str, Any]) -> dict[str, float]:
    ...
    pairs = (
        ("np_state_band", _mapping(urban.get("leader_np")).get("state_band")),
...
def build_priced_wu_link_controller(cfg, tuning: Mapping[str, Any]):
    ...
    globals()["_DUAL_ARM_META"] = install_dual_scale_arm(controller, tuning)
    install_np_state_band()
...
def main() -> None:
    ...
    # 듀얼 스케일 팔의 온디스크 증거. **컨트롤러가 만들어진 뒤**에 써야 한다 — 관측 요약이
    # metadata 를 채우는 자리는 build_priced_wu_link_controller 보다 앞이라 거기서는 항상
    # 비어 있었다(2026-08-26 리플레이에서 `팔 증거 {}` 로 드러났다).
    for _k, _v in (globals().get("_DUAL_ARM_META") or {}).items():
        metadata[_k] = float(_v)
    for _k, _v in ((_NP_BAND_STATE.get("meta")) or {}).items():
        metadata[_k] = float(_v)
    if _NP_BAND_STATE.get("last_error"):
        metadata["np_state_band_last_error"] = str(_NP_BAND_STATE["last_error"])
