...
def _native_fixed_cycle_safe_signals(cfg, plan) -> tuple[list[str], list[str]]:
    """합성 주기가 팽창하지 않는 신호만 고른다.

    러너의 합성 주기는 `녹색합 + 3 x 살아있는 현시수`(VBS:867-871)다. 동시현시로
    녹색합이 주기를 넘는 SC 는 주기가 **팽창한다** — 실측(nativefixed_20260824):

        SC5   97+23+101+25 = 246  ->  240초 (native 150)
        SC7   91+90+0+24   = 205  ->  213초 (native 120)
        SC16  63+17+27+0   = 107  ->  116초 (native 150)

    주기가 1.6배면 그 교차로의 주기당 서비스 횟수가 그만큼 줄고, 인접 신호와 주기가
    달라 연동 자체가 성립하지 않는다. 그래서 nativefixed 의 +201.0 veh·h 를
    "native 분할 + offset 0 의 비용" 으로 읽을 수 없다 — 주기 팽창이 섞여 있다.

    여기서는 팽창하는 SC 를 빼고 나머지만 native 분할로 고정한다. 남는 대조는
    **offset 상실 + 부분창 배달 + 순차 4현시** 이고 주기는 전부 150 으로 같다.
    """
    keep, drop = [], []
    for signal in _controlled_signal_names(cfg):
        ...
        values = [max(0.0, _as_float(greens.get(f"p{p}"), 0.0)) for p in (1, 2, 3, 4)]
        live = sum(1 for v in values if v > 0.0)
        cycle = sum(values) + 3.0 * live
        (keep if cycle <= 150.0 + 1.0e-6 else drop).append(str(signal))
    return keep, drop


def native_fixed_control(cfg, ControlAction, tuning=None):
    ...
    # `urban.native_fixed.cycle_safe_only` 면 주기가 팽창하는 SC 를 빼고, 그 신호는
    # 무제어 기본값(ControlAction.uncontrolled)으로 남긴다.
    section = _mapping(_mapping(_mapping(tuning).get("urban")).get("native_fixed"))
    targets = list(_controlled_signal_names(cfg))
    dropped: list[str] = []
    if _is_enabled_value(section.get("cycle_safe_only")):
        targets, dropped = _native_fixed_cycle_safe_signals(cfg, plan)
    applied = 0
    ...
    control.diagnostics.update({
        ...
        "native_fixed_cycle_safe_only": 1.0 if dropped or _is_enabled_value(
            section.get("cycle_safe_only")) else 0.0,
        "native_fixed_cycle_inflated_dropped": float(len(dropped)),
    })
    return control
...
def main() -> None:
    ...
    try:
        ...
        if args.controller == "no-control":
            ...
            control = native_fixed_control(cfg, ControlAction, tuning)
