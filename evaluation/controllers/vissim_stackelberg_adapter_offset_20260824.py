...
NATIVE_TIMELINE_JSON = WORKSPACE_ROOT / "outputs/signal_timeline_from_sig_20260823.json"


def install_forced_native_offsets(control, tuning: Mapping[str, Any]) -> dict[str, float]:
    """`.sig` 의 신호별 native offset 을 **강제 arm** 으로 싣는다.

    `urban.offset.force_native` 없으면 no-op(비트 동일).

    왜 이게 정당한 경로인가. `offset_promotion` 모듈이 `test_only` 를 둔 목적이 바로
    "**강제 offset arm** 만 낸다" 이다 — 최적화기가 고른 offset 이 아니라 harness 가
    지정한 값. `.sig` 의 native offset 을 그대로 재생하는 것이 정확히 그 경우다.
    **production 은 열지 않는다** — 삼중 잠금과 optimizer-chosen offset 경로는 그대로다.

    왜 스칼라로는 안 되나. 기존 강제 키 둘(`diagnostic_forced_signal_offset_sec`,
    `signal_green_freeze_offset_sec`)은 전 신호에 **같은 값** 하나를 쓴다. 공통 주기
    망에서 균일 이동은 상대 위상을 하나도 안 바꾸므로 연동을 시험할 수 없다. 게다가
    그 둘은 녹색까지 2현시(major/minor)로 얼려 분할을 잃는다.

    격자 정합. native offset 17개가 **전부 정수**다(SC1 131 · SC5 36 · SC7 1 · SC12 5 ·
    SC16 149 · SC101 125 · SC105 35 · SC107 138 · SC108 112 · SC109 111 · SC11 95 ·
    SC6 137 · SC1001~1005 75). 러너는 정수 초에서만 쓰므로 이 값들은 **오차 0** 으로
    들어간다. D-core 의 전이시각 오차는 전적으로 녹색창 경계(실수)에서 오는 것이지
    offset 에서 오지 않는다.
    """
    section = _mapping(_mapping(tuning.get("urban")).get("offset"))
    if not _is_enabled_value(section.get("force_native")):
        return {"forced_native_offset_enabled": 0.0}
    if not NATIVE_TIMELINE_JSON.is_file():
        return {"forced_native_offset_enabled": 0.0, "forced_native_offset_source_missing": 1.0}
    doc = json.loads(NATIVE_TIMELINE_JSON.read_text(encoding="utf-8"))
    signals = _mapping(doc.get("signals")) or _mapping(doc)
    table: dict[str, float] = {}
    for name, entry in signals.items():
        value = _mapping(entry).get("program_offset_sec")
        if value is None:
            continue
        table[str(name)] = float(_as_float(value, 0.0))
    if not table:
        return {"forced_native_offset_enabled": 0.0, "forced_native_offset_empty": 1.0}
    offsets = getattr(control, "offsets", None)
    applied = 0
    if isinstance(offsets, MutableMapping):
        for name, value in table.items():
            if name in offsets or True:
                offsets[name] = float(value)
                applied += 1
    control.diagnostics[offset_promotion.FORCED_ARM_TABLE_KEY] = json.dumps(
        {k: float(v) for k, v in table.items()}, separators=(",", ":"))
    nonzero = sum(1 for v in table.values() if abs(v) > 1.0e-9)
    control.diagnostics["forced_native_offset_applied"] = float(applied)
    return {
        "forced_native_offset_enabled": 1.0,
        "forced_native_offset_applied": float(applied),
        "forced_native_offset_nonzero": float(nonzero),
        "forced_native_offset_max_sec": float(max(table.values())),
    }
...
def write_action_csv(
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        if write_signal_rows:
            # 강제 native offset arm. tuning 절이 없으면 no-op 이라 비트 동일이다.
            _forced_meta = install_forced_native_offsets(control, tuning)
            if isinstance(csv_metadata, MutableMapping):
                csv_metadata.update(_forced_meta)
