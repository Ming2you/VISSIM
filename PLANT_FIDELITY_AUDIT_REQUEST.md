...
**클론 하나면 된다.** 감사에 필요한 모델 소스를 이 레포 안에 스냅샷으로 넣어 두었다.

```bash
git clone -b pstack-flagship-controller https://github.com/Ming2you/VISSIM.git
```

| 위치 | 내용 |
|---|---|
| 저장소 루트 | 하네스·네트워크·스크립트·설정·어댑터 (브랜치 `pstack-flagship-controller`) |
| `vendor/NumSim-mine/src/` | **모델·컨트롤러 소스 사본.** 원본은 https://github.com/Ming2you/Numerical-Sim.git 의 `freeway-zone-followers` 브랜치 `35a5c82` |

`vendor/NumSim-mine/SNAPSHOT.md` 에 사본의 출처·범위·감사 시 볼 파일 목록이 있다.
**사본을 수정하지 말 것** — 런타임이 import 하는 것은 `NUMSIM_REPO_ROOT` 환경변수(없으면
절대경로 폴백)가 가리키는 원본이다(`vissim_stackelberg_adapter.py:24`).

주의 — `.gitignore` 가 `evaluation/runs/` 를 제외한다. 즉 **런 산출물(state/action JSON, 로그)은
레포에 없다.** 4장 수치를 재현하려면 7장의 플랜트 런을 직접 돌려야 하고, 그러려면 PTV Vissim
2020 라이선스가 필요하다. 라이선스가 없다면 **정적 감사**(코드·네트워크 XML·유도 스크립트
대조)만으로도 5장 A/B/D 와 C 의 상당 부분은 답할 수 있다.
