...
def main():
    ...
    result = {'schema': 'saved-snapshot-physical-meter-branches/v1', 'source_sha256': pins,
        ...
        'method': {'stopped_threshold_kph': 1.0, 'stock_is_not_stopped_queue': True,
            ...
            'unknown': 'No route, unavailable or stale route, no explicit future meter, or ambiguous location. No static probability multiplies observed vehicle counts. Unknown lists per meter can overlap and must not be summed.',
    ...
    for sec, state in states.items():
        ...
        for ramp in meters:
            ...
            other, unknown = [], defaultdict(list)
    ...
    for sec, snap in result['snapshots'].items():
        for ramp, values in snap['branches'].items():
            o = ownership[ramp]
            lines.append(f"| {sec} | {ramp} · {o['model_group']} · {o['freeway_owner']} | {values['physical_inventory']}/{values['physical_stopped']} | {values['inventory_before_head']}/{values['stopped_before_head']} | {values['known_upstream_inventory']}/{values['known_upstream_stopped']} | {values['scoped_unknown_count']}/{values['scoped_unknown_stopped']} |")
