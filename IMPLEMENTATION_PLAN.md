> # ⚠ 이 문서는 활성 계획이 아니다
>
> **활성 계획은 [`IMPLEMENTATION_PLAN_V3_LEAN.md`](IMPLEMENTATION_PLAN_V3_LEAN.md) 다.
> 작업을 시작하기 전에 그 문서의 서두를 먼저 읽어라.**
>
> v3 는 이 문서(v2.1)를 **의도적으로 축소**한 것이다. 맥락 상실이나 누락이 아니다.
> 요약하면 — v2.1 은 **물리 충실도**와 **증거 출처 관리**를 함께 추구하는데,
> v3 는 물리 충실도를 한 줄도 줄이지 않은 채 증거 출처 관리(불변 매니페스트, 해시 사슬,
> 재생 검증기, ACL 격리 staging, 서명 번들, wave 폐기 프로토콜)를 걷어냈다.
> 후자는 규제 제출이나 다자 검증의 장치이고 이 연구의 주장이 요구하지 않는다.
>
> **이 문서에 있고 v3 에 없는 항목은 버그가 아니라 결정이다.**
> 되살려야 한다고 판단되면 v3 서두의 "되살리는 것이 옳은 경우" 를 확인하고
> 어떤 주장이 그것을 요구하는지 사용자에게 제시한 뒤 승인을 받아라. 조용히 되돌리지 마라.
>
> 이 문서는 참조용으로 보존한다. 임계치·불변식·게이트 정의의 원본이며 v3 가 그것들을 인용한다.
