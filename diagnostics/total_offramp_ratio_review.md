...
가장 작은 다음 검증인 **동일한 실제 초기 상태·명령·예측 수요·direct shares·capacity를 두고, 별도 cfg 복제본의 네 total ratio만 변경한 한 interval 재실행**을 Flow가 완료했다(`total_offratio_interval_sensitivity.json`). Native totals를 넣은 실제 동적 예측 accepted는266.169대(FE79.206/DE57.840/DW76.951/FW52.174)로 변했다. 위 고정-q 대수합277.541대와는 다른 결과다. 예측 최종 FW stock은1185.298→1071.249대지만 같은1050초 COM 집계927대(앞선 FZP 기록 단계에서는928대)보다 여전히 높다. E8 속도는106.014→106.180km/h로 실측64.285km/h와의 차이가 남는다. 원 cfg/state/demand/action 불변, 같은15step control, 보존 및 생산/data SHA 불변을 확인했으며 이 결과는 calibration 승격이 아닌 prior 민감도다.

후속 native1초 matched-ID 감사도 완료했다(`static_route_matched_cohorts.md/.json`). 600–750초 decision 통과 cohort의 진출 비율은 FE84/193, DE61/119, DW84/205, FW46/148로 native prior에 가까웠고1200초 censoring0이었다. 반면 두 출구 사이 merge cohort는 FE0/7, DE5/10, DW6/6, FW14/15였다. Native conditional prior와 중간 합류 cohort를 구분해야 한다는 직접 관측 근거다. 시점별 source-link departure를 분모로 바꾸는 것만으로는 부족하다. 임의 capacity나 phi, 비율 fitting은 이번 감사 범위에 없다.
