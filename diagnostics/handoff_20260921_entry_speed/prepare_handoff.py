"""Package only the explicit entry-speed continuation; preserve previous archives."""
...
def main():
    ...
    selected = set()
    for p in list(K.glob('ramp_entry_velocity*')) + [K / 'route_state_native_v1/rm_ramp_s23']:
        selected.update([p] if p.is_file() else (v for v in p.rglob('*') if v.is_file()))
    direct = {ROOT / p for p in changed}
    direct.update(K / name for name in ('RAMP_ENTRY_VELOCITY.md', 'test_ramp_entry_velocity.py', 'verify_ramp_entry_velocity.py'))
    ...
    manifest = dict(schema='deduplicated-git-handoff/v1',
        ...
        requires_package='diagnostics/handoff_20260921_response', files=records, parts=parts,
        unique_objects=len(objects), archive_bytes=archive.stat().st_size, excluded_raw=excluded,
        excluded_note='Size/location inventory, not full raw file hashes. Original comparison FZP is also required to repeat raw equality; see analysis_v2/result.json source_receipts.')
    ...
    marker = '# 2026-09-21 entry-speed handoff byte preservation'
    ...
    rules += ['"diagnostics/handoff_20260921_entry_speed/**" -text whitespace=cr-at-eol']
