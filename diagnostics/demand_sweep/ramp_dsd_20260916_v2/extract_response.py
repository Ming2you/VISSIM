...
def main():
    ...
    parser.add_argument('--terminal-sec',type=int,choices=(1050,3000,4500),default=3000)
    parser.add_argument('--arms',nargs='+',choices=('none','rm','vsl','both'),help='Extract only specified completed runs')
    parser.add_argument('--runs',type=Path)
    parser.add_argument('--output',type=Path)
    parser.add_argument('--nested-runs',action='store_true',help='Read arm/run instead of run_arm')
    parser.add_argument('--component-stocks',action='store_true',help='Retain mainline/on/off inventory at the declared native recording interval')
    args=parser.parse_args()
    if args.pairs and args.arms:parser.error('Pair extraction reads arms from its protocol')
    ...
    for arm in (tuple(pair_protocol['candidate_bank']) if args.pairs else (args.arms or ('none','rm','vsl','both'))):
        ...
        interval=ex.load(Path(receipt['prepared'])/'prepared.json').get('vehicle_record_interval_sec',1)
        observer,ports=ex.Observer(g,removals,interval_sec=interval),ex.PortObserver(g,interval_sec=interval)
        ...
        for sec,current in native_frames(run/'vissim_eval/baseline_001.fzp',evidence,deadline=time.monotonic()+1800,interval_sec=interval):
            ...
            for no,row in current.items():
                ...
                if link in (74,26) and 0<=pos<2400:
                    ...
                    c=spatial[key];c[0]+=interval;c[1]+=speed*interval;c[2]+=(speed<30)*interval
            for no,before in previous.items():
                ...
                if before[2]<=head and (row[0]!=before[0] or row[2]>heads[row[0]][row[1]]):
                    ...
                    if interval!=1:crossings[-1].update(lower_time_s=sec-interval,upper_time_s=sec)
            for ramp in heads:
        ...
        for name,rows in [('cells_30s',observer.cells),('flows_30s',observer.flows),('boundaries_30s',observer.boundary_rows),('ports_30s',ports.rows),('port_events',ports.events),('head_crossings',crossings),(f'head_stock_{interval}s',head_stock)]:table(out/(name+'.csv'),rows)
        ...
        if args.component_stocks:table(out/f'component_stocks_{interval}s.csv',component_stocks)
        save(out/'manifest.json',{'source_run':str(run),'source_receipt_sha256':sha(run/'run.json'),'network_sha256':sha(network),'fzp':evidence,'cell_conservation_checks':observer.checks,'exclusions':observer.evidence,'removals':removals,'extractor_sha256':sha(Path(__file__)),'complete':True,
            'observation_interval_sec':interval,'head_crossings_scope':'Observed prehead vehicle followed beyond head; passages entirely between frames can be missing.',
            'sparse_limits':'Event times are interval brackets; sampled residence is approximate; sparse FZP alone does not certify complete Omega TTD.' if interval>1 else None})
