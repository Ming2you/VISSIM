...
class FIFO:
    ...
    def take_label(self, label, amount):
        ...
        if amount < -EPS or amount > self.counts()[label]+1e-7:
        ...
        if self.lateral_access:
            ...
            for target, requests in lateral_requests.items():
                ...
                for key, label, demand in requests:
                    n = demand*fraction
                    if n <= EPS: continue
                    packets = self.cells[key].take_label(label, n)
        ...
        while active:
            ...
            for key in sorted(active, key=str):
                ...
                target, reason = ((external[key[1]][0], 'entry') if key[0] == 'external'
                                  else self.route(key, q.q[0][0], receiving))
                if target is None:
                ...
                demand = 0.
                for label, n in q.q:
                    next_target = external[key[1]][0] if key[0] == 'external' else self.route(key,label,receiving)[0]
                    if next_target != target: break
                    demand += min(n, sending[key]-demand)
                    if demand >= sending[key]-EPS: break
            ...
            for target in sorted(requests, key=str):
                ...
                fraction = 1. if target[0] == 'exit' else min(1., receiving[target]/total)
