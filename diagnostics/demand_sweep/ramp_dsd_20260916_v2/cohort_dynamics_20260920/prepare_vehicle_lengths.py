...
def main():
    ...
    ap.add_argument('--case',choices=['none_s23','vsl_s23'],default='none_s23');args=ap.parse_args()
    ...
    attrs=root.find('./evaluation/vehRec/attributes');assert len(attrs)==10
    ...
    assert profile['seed']==23 and not profile['meter_commands']
    ...
    e.save(out/'protocol.json',{'seed':23,'end_s':3000,'source':str(source.relative_to(e.ROOT)),
        ...
        'commands':profile['vsl_commands'],'reference_run':str(reference.relative_to(e.ROOT)),
        'required_gate':'All original10 FZP fields exactly equal through3000, native signal validation, nonempty added attributes',
