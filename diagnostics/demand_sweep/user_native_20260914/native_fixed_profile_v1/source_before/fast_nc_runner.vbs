...
End If
If WScript.Arguments.Count <> 4 And WScript.Arguments.Count <> 5 And WScript.Arguments.Count <> 6 Then WScript.Echo "Usage: network prepared output seed [terminal_sec] [native_preserve]": WScript.Quit 2
...
If WScript.Arguments.Count = 6 Then
    If WScript.Arguments(5) <> "native_preserve" Then WScript.Echo "ERROR=Invalid mode": WScript.Quit 2
    nativePreserve = True
End If
...
If nativePreserve Then RunPreservedNative
...
Sub RunPreservedNative()
    Dim saved, fields, actual, checks
    Set sim = CreateObject("Vissim.Vissim")
    WScript.Echo "STAGE=COM_CREATED"
    sim.LoadNet net, False
    WScript.Echo "STAGE=NET_LOADED"
    ...
    Loop
    saved.Close
    If checks <> 3 Or CDbl(sim.Simulation.AttValue("RandSeed")) <> seed Then Die "Native simulation identity"
    On Error Resume Next
    sim.Graphics.CurrentNetworkWindow.AttValue("QuickMode") = 1
    sim.SuspendUpdateGUI
    Err.Clear
