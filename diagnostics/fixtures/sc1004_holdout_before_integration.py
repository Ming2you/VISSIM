...
from diagnostics.prepare_sc1004_resource_service_patch import proposal_module
...
CALIBRATED='diagnostics/route_choice_corridor_sc1004_calibrated_proposal.json'
...
def main():
    ...
    program=geometry()[2];module=proposal_module();started=time.monotonic();rows=[]
    ...
    paths=[Path(__file__),CONFIG,ROOT/CALIBRATED,ROOT/'diagnostics/sc1004_resource_service_calibration.json',
           ROOT/'diagnostics/prepare_sc1004_resource_service_patch.py',run/'action_000001.json',run/'state_000900.json',run/'anchor_002700.json']
    tuning=load(CONFIG)
    ...
    for second in (900,2700):
        ...
        for label,calibrated in [('inherited',False),('train_lower_bound',True)]:
            ...
            with tempfile.TemporaryDirectory(dir=ROOT/'diagnostics') as directory:
                ...
                with patch.object(rc,'_configure_one',module._configure_one):
    ...
    (ROOT/'diagnostics/sc1004_resource_service_holdout.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
