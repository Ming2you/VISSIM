...
if (-not $Execute) {
  [pscustomobject]@{execute=$false; program=$cscript; arguments=$arguments; startup_no_progress_sec=300;
    controller='none'; vehicle_queries=0; initial_native_steps=1; continuous_calls=1; terminal_sec=$terminalSec} | ConvertTo-Json
  exit 0
}
...
while (-not $runner.HasExited) {
  ...
  if (-not $progress) {
    foreach ($file in @(Get-ChildItem -LiteralPath $evalPath -Filter '*.fzp' -File)) {
      ...
      if ($null -ne $lastSim -and $lastSim -gt 1) {
    ...
    if (-not $progress -and ((Get-Date)-$started).TotalSeconds -ge 300) {
      $timedOut=$true; $record.error='Startup300s: no complete FZP data timestamp beyond native first step'
