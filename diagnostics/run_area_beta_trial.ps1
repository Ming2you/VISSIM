...
if ($SimPeriod % 150 -ne 0) { throw 'SimPeriod must end on a 150-second boundary' }
$areaRepo = Split-Path -Parent $PSScriptRoot
Set-Location -LiteralPath $areaRepo
$areaConfig = "diagnostics/$ConfigDirectory/n7_area_beta$BetaSeconds.json"
$areaOutput = Join-Path 'evaluation/runs' $Name
if (Test-Path -LiteralPath $areaOutput) { throw "Run output already exists: $areaOutput" }
$areaManifestPath = Join-Path $PSScriptRoot "$ConfigDirectory/manifest.json"
...
function Assert-AreaSources {
  ...
  if ($selected.path.Replace('\','/') -ne $areaConfig) { throw 'Candidate config path differs from selected manifest output' }
