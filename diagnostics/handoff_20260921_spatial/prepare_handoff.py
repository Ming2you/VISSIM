"""Package the explicit spatial continuation; preserve previous archives."""
...
def main():
    ...
    selected = set()
    prefixes = ('paired_native_interaction', 'downstream_spatial', 'downstream_boundary', 'local_interaction_spatial')
    ...
    direct.update(K / name for name in ('PAIRED_INTERACTION_AND_SPATIAL_RESPONSE.md', 'verify_local_interaction_spatial.py'))
    ...
    manifest = dict(schema='deduplicated-git-handoff/v1',
        ...
        requires_package='diagnostics/handoff_20260921_entry_speed', files=records, parts=parts,
        unique_objects=len(objects), archive_bytes=archive.stat().st_size, excluded_raw=excluded,
        excluded_note='Local spatial evidence only. Raw FZP dependencies remain excluded by previous packages; saved frames, aggregates and boundary sequences support read-only checks.')
    ...
    marker = '# 2026-09-21 spatial handoff byte preservation'
    ...
    rules += ['"diagnostics/handoff_20260921_spatial/**" -text whitespace=cr-at-eol']
