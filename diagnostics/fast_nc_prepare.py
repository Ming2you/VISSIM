...
def prepare_native_preserve(network, output, terminal, *, recording_interval_sec=5, fzp_only=False):
    ...
    if simulation is None or int(simulation.get('numRuns')) != 1:
        raise ValueError('Native-preserve requires a saved single-run network')
    seed = int(simulation.get('randSeed'))
    if seed < 1 or number(simulation.get('simRes')) <= 0:
        raise ValueError('Invalid saved seed or simulation resolution')
    assets = {}
        ...
        for value in node.attrib.values():
            if value.startswith('#data#'):
                ...
                if not name or Path(name).name != name or '/' in name or '\\' in name:
                    raise ValueError('Expected sibling native asset: ' + value)
                assets[name] = (network.parent/name).read_bytes()
    output.mkdir(parents=True, exist_ok=False)
    snapshot = output/'network'
    snapshot.mkdir()
    ...
    print(json.dumps({'prepared':str(output.resolve()), 'network':str(copied.resolve()),
                      'mode':'native_preserve', 'source_sha256':digest}))
