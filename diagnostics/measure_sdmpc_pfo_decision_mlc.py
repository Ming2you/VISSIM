"""Offline PFO + multi-budget central SDMPC decision (handoff item 3 verification).

Same as measure_sdmpc_pfo_decision.py except the one-candidate pin is relaxed so the
leader budget search actually runs. Everything else, including the source pins, is identical.
"""
...
def main():
    ...
    # Variant of measure_sdmpc_pfo_decision.py for handoff item 3 (multi-budget search).
    # The original pins max_leader_candidates==1 so its recorded measurement matches the
    # documented one; here we allow more candidates and record the actual count instead.
    if options['response_parallel_workers']!=8:
        raise ValueError('Expected eight workers')
    if options['max_leader_candidates']<2:
        raise ValueError('This variant is for verifying the multi-budget search; use >=2 candidates')
