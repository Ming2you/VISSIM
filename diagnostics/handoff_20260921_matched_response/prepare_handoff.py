"""Package the explicit matched-response continuation; preserve previous archives."""
...
def main():
    ...
    selected = set()
    prefixes = ('matched_resolution_response', 'matched_lane_response', 'matched_port_timing_response', 'matched_seed33_boundary')
    ...
    direct.update(K / name for name in ('MATCHED_SPATIAL_RESPONSE.md',))
    ...
    manifest = dict(schema='deduplicated-git-handoff/v1',
        ...
        requires_package='diagnostics/handoff_20260921_midpoint', files=records, parts=parts,
        unique_objects=len(objects), archive_bytes=archive.stat().st_size, excluded_raw=excluded,
        excluded_note='Matched RM development forecasts and retrospective accounting only. No raw native files selected in this increment. Earlier packages provide observations and native validation. Candidate switches remain unqualified.')
    ...
    marker = '# 2026-09-21 matched response handoff byte preservation'
    ...
    rules += ['"diagnostics/handoff_20260921_matched_response/**" -text whitespace=cr-at-eol']
