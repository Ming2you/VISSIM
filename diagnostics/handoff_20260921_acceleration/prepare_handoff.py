"""Package the completed acceleration diagnostics, not raw native recordings."""
...
def main():
    ...
    expected = {'AGENTS.md', 'docs/HANDOFF_20260921_gain_prediction.md',
        ...
        'diagnostics/handoff_20260921_transport/verify_records.py'}
    assert changed == expected, (sorted(changed - expected), sorted(expected - changed))
    direct = {ROOT / p for p in changed}
    direct.update(K / name for name in ('INTERACTION_MEMORY_AND_TARGET_RESPONSE.md', 'SPATIAL_ACCELERATION_AND_AUTONOMOUS_GATE.md'))
    prefixes = ('interaction_duration_audit', 'target_history_audit', 'target_spatial_representation',
        'target_acceleration_coupling', 'physical_response_scale', 'check_response_scale',
        'verify_target_spatial_coupling', 'verify_interaction_memory')
    selected = set()
    ...
    manifest = dict(schema='deduplicated-git-handoff/v1',
        ...
        requires_package='diagnostics/handoff_20260921_transport', files=records, parts=parts,
    ...
    marker = '# 2026-09-21 acceleration response handoff byte preservation'
    ...
    rules.append('"diagnostics/handoff_20260921_acceleration/**" -text whitespace=cr-at-eol')
