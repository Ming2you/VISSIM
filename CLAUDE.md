...
2. **~~`urban_movements` 에 on_ramp movement 를 추가하지 마라.`~~ 2026-09-03 승격으로 철회.**
   기각 사유 셋 중 하나만 참이었고, 그것도 원인 진단이 틀렸다.
     - beta 손상은 실재한다(형제 17개가 25~50% 깎임). 단 원인은 'on_ramp 추가' 가 아니라
       **`build_urban_movements` 의 균등 유도 beta 로 실측 정본을 덮은 것**이다. 새 movement 만
       넣고 기존 beta 를 `(1-램프몫)` 배로 비례축소하면 형제 상대비가 보존된다(351쌍 검사 깨짐 0).
     - `kind=boundary_in` 6개는 결함이 아니다. 그 6개는 실제로 경계 게이트 in링크
       (`in_SC1001_W` 등)에서 출발하므로 boundary_in 이 옳은 계상이다.
     - 이중계상은 실측으로 반박됐다. 미터 전폐 1구간에서 램프행 순증 183.2 veh 대
       외생 도착 165.7 veh = 초과 10.6%. '재고÷drain 이중계상' 이면 2배여야 한다.
   **왜 켜야 하나.** 안 켜면 롤아웃 램프 저수지에 유입 경로가 하나도 없다 — `on_ramp_to_movement`
   공집합이 `urban_queue_model.py:1134` 의 `or not movements` 가드에 걸려 외생 도착을 통째로
   버리고(37결정 5,393.5 veh 소멸), metanet 경로는 `coupling.py:214 update_ramp_queues=False`
   로 죽어 있다. 검정: `demand.ramp_arrival` 을 0 으로 두든 3976 vph 로 채우든 궤적이 비트 동일.
   **세 조각이 함께여야 한다** (폐루프 실측, 기준선 ctl_outflow 8822.786):
     주입만                        8909.0  **+86.2 악화**
     주입 + 램프관측 커넥터 8개      8432.8  -390.0
     주입 + 관측 + beta 비례축소     8339.0  **-483.8 (-5.48%)**  <- 정본
   주입만으로 악화하는 이유는 관측이 과대해서다 — `ramp_link_to_queues` 가 도시 피더 링크
   4개(31·68·78·10703)를 100% 램프 큐로 세어 R_D_W 22/37 · R_F_E 12/37 에서 상한을 넘겼고,
   그 상태로 유입을 켜면 `urban_queue_model.py:1197` 의 `min(_cap_r, ...)` 절단이 차량을 지운다.
   미터 커넥터 8개만 세면 네 램프 모두 초과 0/37 이다(상한이 틀린 게 아니라 관측이 과대였다).
...
### 죽은 현시에 붙은 movement 9개 — 해소 (2026-09-01)

`apply_movement_phase_correction` 이 11개를 옮겨도 9개가 남아 영원히 녹색을 못 받았다.
커넥터 지도의 **실제 SG** 로 대조해 두 종류로 갈렸다.

**현시 배정 오류 6건** — NEMA **이름 규칙**이 접근로 방위로 현시를 정해 실측 SG 와 어긋났다.

```
SC107_N_SC1_to_S · _to_W_SC1004 · _to_W_SC1005   p1 -> p2   N_SC1 진출 10103·10610 이 sg=[7] SBL
SC107_S_to_E_SC108 · _to_N_SC1                   p1 -> p2   S 진출 10618 이 sg=[3] NBL
SC16_W_to_N_SC12                                 p4 -> p3   W 진출이 전부 sg=[2] EBT
```

SC107 은 `sg4 SBT = 0 · sg8 NBT = 0` 이라 **남북 직진이 없는** 교차로다. 그래서 p1 이 죽었고,
N/S 접근로는 좌회전 현시(p2, 녹색 25)로만 서비스된다. SC16 은 **동서 직진·좌회전이 한 신호**라
좌회전 전용 p4 가 실측 0 이다.

**물리 회전이 없는 movement 3건** — leg 교차곱 산물. `apply_nonexistent_movement_beta_zero`
가 beta 를 0 으로 하고 같은 origin 의 형제에게 넘긴다(합 1.0 유지).
