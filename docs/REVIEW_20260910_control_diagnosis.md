...
| 축 | 의심과 확인 근거 | 판정·조치 |
|---|---|---|
| ① 수요 | plant는 6개 900초 구간의 지정 배수를 적용한다. 기존 예측은 현재율 지속이었으며 내부 입력 9곳의 미래 발생이 빠져 있었다. 누락 규모는 1350초 설정 수요 1960대/시간이다 | 내부 입력 10곳 전체의 발생과 신호·경로를 연결하고, 검증된 시간표를 예측 구간별로 적분하도록 수정했다. 초기 재고, 원하는 발생량, 실제 수용량, 미수용 대기를 구분한다. 116개 복원 검사와 두 전체 MPC 검증을 통과했고 전체 제어 실런을 진행한다 |
...
해당 결과 이후 추가 내부 입력 9곳의 누락을 확인해 수정했고, 아래 통합 검증을 마친 뒤 제어 실런을 재개했다.
...
이용해 두450초 검증 구간을 제외한 자료로 추정했다. 보정 후 같은 구간 방출은34.67대와33.51대다.
두 입력의 모형 미수용 대기는0으로 줄었다. 실제 발생은26대와48대, 모형의 평균 수요는35대씩이므로
방출 오차 전체를 서비스율만의 오차로 해석하지 않는다. 아직 seed13 내 시간 분리 검증이다.
...
코드324a3b8을 고정하고 `codex_area_sources_beta0_s13_20260910`을2026-09-10 08:13:13 KST에
시작했다. Ver2·seed13·5400초,900초부터150초마다 네 가지 레버를 최적화한다.
시작 후300초 동안 시뮬레이션이 진행되지 않으면 해당 실행에서 식별한 VISSIM만 종료한다.
이 팔의 성능 수치는 완료·측정·실제 명령 검증을 마친 후 추가한다.

첫900→1050초의 [실제 명령·예측 감사](../diagnostics/source_interval_900_1050.md)는 통과했다.
현재 작업 소스가 실행 manifest와 같고, 실제JSON을 직렬화한213개 물리 명령행이 CSV와 일치한다.
신호130개 그룹의1초 기록과66개 VSL 명령의 두 차량종류 readback을 검사했다. 시작초에
램프8개를 동일하게 두 번 적용한 기록만 명시적으로 허용하고, 누락·상충·NaN은 거부한다.
그러나 ΩTTT는 예측85.0253/관측79.9181 veh·h, Ω유출은442.756/710대였다.
관측 유출710대는 직접 경계483대와 종단 추론227대이며 내부 소실4대는 제외했다.
고속도로 체류시간 오차+6.5945와 나머지 Ω의−1.4873 veh·h가 부분 상쇄된다.
1050초 COM 본선 재고도1176.087/917대다. 이 결과는 실제 첫 명령을 유지한 예측 오차이며,
내부 발생 수선의 단독 효과나 최종β0 성능으로 해석하지 않는다.

![수정 모델의 실제 첫 구간 예측 오차](../diagnostics/source_interval_900_1050_figure.png)

[제어 전 동일성 감사](../diagnostics/source_run_warmup_equivalence.json)에서는870초까지
관측30행·도로6691행·본선셀1260행과900초 전체2591대의 위치·속도·경로가 NC와 같았다.
다만 관측 이력은 다르다. 실제 MPC warmup은150초마다 관측창을 끝내고, NC는900초까지
긴 창을 유지하므로 local_observation을 같은 값으로 간주하지 않는다. 첫 명령 재생은 실제
750초 명령과900초 raw 관측을 사용했다. e1a8114는 두 런처의 기존 작업 바이트를
Git에도 보존하는 후속 커밋이며 실행 중인 파일을 바꾸지 않았다.126개 소스·4개 config의
실제 Git checkout 바이트가 모두 manifest와 일치함을 별도로 검증했다.

[1200→1350초의 실제 명령 재생](../diagnostics/source_interval_1200_1350.md)에서도
명령·관측·보존 검증은 통과했지만 E8 끝점 속도는 예측96.116/관측40.485km/h,
차량 수는33.313/66대였다. E8의 실제30초 관측은 전 구간31–40km/h인 반면,
모형은 첫10초부터60km/h 이상으로 회복한다. E9는 실제로 잠시 회복했다가
1350초에43.168km/h로 다시 느려졌고, 모형은84.791km/h를 유지했다.
42셀 평균 속도 오차 하나로 묶으면 다른 셀의 느린 예측이 이 국소적인 과대 회복을 가린다.
동일한 원자료의 [셀별 수치](../diagnostics/source_interval_1200_1350_cells.csv)와
10초 예측·30초 실제 관측을 함께 보존했다. 다음 그림은 용량 추정이나 제어 효과의 단독 인과 추정이 아니다.

![동일 제어 명령에서 E8·E9의 실제 변화와 예측](../diagnostics/source_interval_1200_1350_local_cells.png)

실제1200초의 램프 폐쇄를 [동일 명령 개입 비교](../diagnostics/source_meter_shutdown_diagnosis.md)로
분리했다. DW/FE만 다시 열고 다른 명령을 유지하면450초 모형의 ΩTTT는
390.650860→389.457098 veh·h, TTD는1594.104→1636.333대로 좋아진다.
그러나 Ω endpoint 이후 리더가 기존 본선 밀도 벌점을 다시 더한다. 이 벌점이
59.807598→69.729835 veh·h로 커져 총비용의 순위가 반대로 바뀐다.
이는 VISSIM 개선의 증거가 아니라 **현재 최적화가 요청한 Ω 목적함수와 다른 값을
최적화한다는 직접적인 오류 증거**다. all_urban_halfcap 벌점도 Ω 밖의 boundary를
포함한 전체 모형 저장소를 본다. 현재 실런을 완료한 뒤 full/proxy/fallback 공통
경로에서 목적 외 가산을 제거하고, 물리적 저장공간·수신 제약은 유지해 재검증한다.

신호 추정에는 [용량 설치 감사](../diagnostics/selected_signal_capacity_audit.md)와
[표본 간격 감사](../diagnostics/selected_signal_sampling_audit.md)를 별도로 수행했다.
7개 도로의720→870초 이탈량은 동일1초 기록335대/30초 표본270대이며, 후자는
실제 raw900 COM 계수와 모두 일치한다. 누락65대 중 확인된 head 경로14대,
신호 전 우회·head 없는 차로41대, 차로 전이 미확정10대다. link 이탈 전수를
신호 서비스로 해석하면 안 된다. 이탈창은 decision보다30초 늦게 끝나는 것이 아니라
**30초 전에 끝나며**, 녹색 분모에는 warmup에서 실행되지 않은 JSON 기본값이 쓰였다.
실제 정지선·차로·위치의 통과와 동일 창의 SG GREEN 노출을 함께 수집해야 한다.

SC1004의10634는 세 source가 공유하는 물리 회전이다. 같은 실제900초 초기 상태의
[한 스텝 비교](../diagnostics/local_shared_service_contract_review.md)에서 전역 모형은
5초 예산0.860544대만 쓰고, 국소 ramp-aware 모형은 세 source가 각각 그 예산을 써서
2.581633대를 내보냈다. receiver 공간은 충분했고 공유 pool 누락을 직접 재현했다.
이는 국소 서비스 검증이며 follower 전체 점수나 실제 유량의 세 배 오차로 일반화하지 않는다.

[native 진출 비율의 같은 명령 비교](../diagnostics/source_native_offratio_sensitivity_1200.md)는
본선 끝 재고 오차를1285.088→1142.102대(관측1115대)로 줄였다. 그러나 그룹별
과대·과소 방출이 상쇄되고 E8 속도는98.866km/h로 실제40.485를 계속 크게 벗어났다.
진출 비율의 단순 치환을 최종 수선으로 채택하지 않는다.
...
[3300→3450초의 실제 명령 재생](../diagnostics/source_interval_3300_3450.md)에서도
실행 소스·명령·1초 관측·보존 검증을 통과했다. ΩTTT는 예측223.728728/실제215.347222
veh·h로 본선과 도시부 모두 과대 예측했다. 동시에 E8의 끝 속도는 예측12.886/실제1.952km/h,
재고는157.972/289대였다. E9는 예측24.226/실제3.303km/h다. 본선 전체 체류시간을
과대 예측하면서도 병목에 쌓인 차량과 그곳의 정체 지속은 과소 예측할 수 있다.
두 셀의 끝점은 모두 정체 상태이며, 이 결과를 자유류 회복 예측이라고 부르지 않는다.

![심한 정체에서 E8·E9의 실제 변화와 예측](../diagnostics/source_interval_3300_3450_local_cells.png)

[유출 점수의 경로별 분해](../diagnostics/source_exit_attribution_3300_3450_verified.md)는
동일 endpoint 하나의 TTT·TTD·유입·8814개 수락 사건과 모든 유량 계수를 그대로 보존했다.
모형 TTD465.384대 중 본선 terminal이232.194대이고 나머지가233.190대다.
실제는 관측 경계 유출540대와 terminal 추론221대를 별도 기록했다.
경로가 유일하고 경계 통과 수가 수락 유량과 일치하는 movement만 물리 경계에 대응시켰다.
나머지는 미대응으로 남겼으며, 1초 기록의 connector 생략 때문에 표본 경계 쌍과
모형의 경계 쌍이 다를 수 있다는 한계도 유지했다. 이 분해는 용량 조정의 효과가 아니라
TTD 오차가 어느 경로에서 생기는지 추적하기 위한 감사다.

## 기존 β0의 5400초 완료 결과

[전체 공간·제어 평가](../diagnostics/source_run_comparison_complete/assessment.md)에서 기존 β0는 무제어보다 ΩTTT가381.270 veh·h(전체6.834%) 늘고 TTD는1378사건 줄었다. 30개 제어 구간의 실제 명령 검증은 모두 통과했다. E8 시작은90초 늦었지만 E0 도달은2730초로 같고, E9와 W0는 더 일찍 정체됐다. 목적함수 및 서비스 계약 수정 뒤 실제 실행을 다시 검증하며, 이 버전은 최종 정본으로 채택하지 않는다.
